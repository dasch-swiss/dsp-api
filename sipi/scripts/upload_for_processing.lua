...
local tmpFolder = config.imgroot .. '/processed/'
local exists
...
for file_index, file_params in pairs(server.uploads) do
    ...
    local tmp_storage_file_path = config.imgroot .. '/processed/' .. hashed_tmp_storage_filename
    local tmp_storage_sidecar_path = config.imgroot .. '/processed/' .. hashed_tmp_storage_sidecar
    local tmp_storage_original_path = config.imgroot .. '/processed/' .. hashed_tmp_storage_original

    -- Create a IIIF base URL for the converted file.
    local tmp_storage_url = get_external_protocol() .. "://" .. get_external_hostname() .. ":" .. get_external_port() .. '/processed/' .. tmp_storage_filename
    ...
    if media_type == IMAGE then
        ...
        if not success then
            send_error(500, "upload_for_processing.lua: uploaded_image:mimetype_consistency() failed: " .. check)
            return
        end
        if not check then
        ...
        if not success then
            ...
            send_error(500, "upload_for_processing.lua: normalize image orientation failed for: " .. tostring(tmp_storage_file_path) .. ": " .. tostring(error_msg))
            return
        end
        ...
        server.log("upload_for_processing.lua: wrote image file to " .. tmp_storage_file_path, server.loglevel.LOG_DEBUG)
    ...
    elseif media_type == VIDEO then
        ...
        server.log("upload_for_processing.lua: wrote video file to " .. tmp_storage_file_path, server.loglevel.LOG_DEBUG)
        ...
        server.log("upload_for_processing.lua: wrote non-image file to " .. tmp_storage_file_path, server.loglevel.LOG_DEBUG)
    end
    ...
    if media_type == VIDEO then
        ...
        if not duration then
            send_error(417, "upload_for_processing.lua: ffprobe get duration failed: " .. duration)
        end
        -- get video width (dimX)
        local width = tonumber(file_meta_json['width']);
        if not width then
            send_error(417, "upload_for_processing.lua: ffprobe get width failed: " .. width)
        end
        -- get video height (dimY)
        local height = tonumber(file_meta_json['height'])
        if not height then
            send_error(417, "upload_for_processing.lua: ffprobe get height failed: " .. height)
        end
        -- get video fps
        ...
        if not frames then
            send_error(417, "upload_for_processing.lua: ffprobe get frames failed: " .. frames)
        else
            fps = frames / duration
            if not fps then
                send_error(417, "upload_for_processing.lua: ffprobe get fps failed: " .. fps)
            end
        end
