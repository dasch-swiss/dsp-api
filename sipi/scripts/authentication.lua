...
--- Extracts a JSON web token (JWT) from the HTTP request:'Authorization' header or query param 'token'.
...
--- Extracts a JSON web token (JWT) from the HTTP request:'Authorization' head or query param 'token'.
-- If present decodes token and validates claims exp, aud, iss.
-- Sends an HTTP error if the token is invalid.
