...
object PermissionRestServiceE2ESpec extends E2EZSpec {
  ...
  private def failsWithA[A <: Throwable](msg: String)(implicit tag: ClassTag[A]): Assertion[Exit[Any, Any]] =
    fails(isSubtype[A](hasMessage(containsString(msg))))
  ...
  val e2eSpec = suite("PermissionResponder")(
    suite("Administrative Permission Create Requests")(
      test("return 'BadRequest' if the supplied project IRI for AdministrativePermissionCreateRequestADM is not valid") {
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException]("Project IRI is invalid.")))
      },
      test("return 'BadRequest' if the supplied group IRI for AdministrativePermissionCreateRequestADM is not valid") {
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException](s"Invalid group IRI $groupIri")))
      },
      test(
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException](s"Invalid permission IRI: $permissionIri.")))
      },
      test("return 'BadRequest' if the no permissions supplied for AdministrativePermissionCreateRequestADM") {
        ...
        permissionRestService(
          ...
          assert(_)(
            failsWithA[BadRequestException](
      ...
      test(
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException]("Permissions needs to be supplied.")))
      },
      test(
    ...
    suite("Default Object Access Permission Create Requests")(
      ...
      test(
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException](s"Group IRI is invalid: $groupIri")))
      },
      test(
        ...
        ).exit.map(assert(_)(failsWithA[BadRequestException](s"Couldn't parse IRI: $permissionIri")))
      },
      ...
      test(
        ...
        ZIO
          ...
          .map(
            assert(_)(
              failsWithA[BadRequestException](s"Given permission code 2 and permission name CR are not consistent."),
            ),
          )
      },
      test(
