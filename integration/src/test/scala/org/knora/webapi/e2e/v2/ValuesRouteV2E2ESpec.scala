...
class ValuesRouteV2E2ESpec extends E2ESpec {
  ...
  private def getResource(resourceIri: IRI): JsonLDDocument = {
    val cred       = BasicHttpCredentials(SharedTestDataADM.rootUser.email, SharedTestDataADM.testPass)
    val iriEncoded = URLEncoder.encode(resourceIri, "UTF-8")
    val request = Get(
      baseApiUrl + s"/v2/resources/$iriEncoded",
      HttpEntity(ContentTypes.`application/json`, "")
    ) ~> addCredentials(cred)
    val response: HttpResponse = singleAwaitingRequest(request)
    val responseString: String = responseToString(response)
  ...
  private def createTextValueWithStandardStandoffRequest(resourceIri: IRI, textValueAsXml: String)(implicit
  ...
  private def createTextValueWithCustomStandoffRequest(resourceIri: IRI, textValueAsXml: String, mapping: String)(
    implicit stringFormatter: StringFormatter
  ): String =
    s"""{
       |  "@id" : "$resourceIri",
       |  "@type" : "anything:Thing",
       |  "anything:hasCustomFormattedText" : {
  ...
  "The values v2 endpoint" should {
    ...
    "create a text value with standoff test1" in {
      ...
      val jsonLDEntity = createTextValueWithStandardStandoffRequest(resourceIri, textValue1AsXmlWithStandardMapping)
    ...
    "create a very long text value with standoff and linked tags" in {
      ...
      val jsonLDEntity = createTextValueWithStandardStandoffRequest(resourceIri, textValueAsXml)
    ...
    "create a TextValue from XML representing HTML with an attribute containing escaped quotes" in {
      ...
      val resourceIri: IRI                          = AThing.iri
      val propertyIri: SmartIri                     = "http://0.0.0.0:3333/ontology/0001/anything/v2#hasCustomFormattedText".toSmartIri
      val maybeResourceLastModDate: Option[Instant] = getResourceLastModificationDate(resourceIri, anythingUserEmail)
      ...
      val jsonLDEntity = createTextValueWithCustomStandoffRequest(
        resourceIri = resourceIri,
        textValueAsXml = textValueAsXml,
        mapping = s"${SharedTestDataADM.anythingProjectIri}/mappings/HTMLMapping"
      )
      ...
      "create, update and delete an UnformattedTextValue" in {
        ...
        val requestString =
          s"""{
             ...
             |    "@type" : "knora-api:UnformattedTextValue",
             |    "knora-api:valueAsString" : "$textValue"
             |  },
             |  "@context" : {
             |    "knora-api" : "http://api.knora.org/ontology/knora-api/v2#",
        ...
        // update the value
        val updatedTextValue = "This is an updated text value."
        val updateRequestString =
          s"""{
             ...
             |    "@id" : "$valueIri",
             |    "@type" : "knora-api:UnformattedTextValue",
             |    "knora-api:valueAsString" : "$updatedTextValue"
             |  },
             |  "@context" : {
             |    "knora-api" : "http://api.knora.org/ontology/knora-api/v2#",
             |    "freetest" : "http://0.0.0.0:3333/ontology/0001/freetest/v2#"
...
object TextValuesTestData {
  val resourceIri = "http://rdfh.ch/0001/zNI_RQzqRSOKQkgPUNhjDw"
}
