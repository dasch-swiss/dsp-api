  ...
  "The UsersRestService" when {
    "calling getAllUsers" should {
      "with a SystemAdmin should return all real users" in {
        val response = UnsafeZioRun.runOrThrow(UsersRestService(_.getAllUsers(rootUser)))
      ...
      "fail with unauthorized when asked by an anonymous user" in {
        val exit = UnsafeZioRun.run(UsersRestService(_.getAllUsers(SharedTestDataADM.anonymousUser)))
    ...
    "calling getUserByEmail" should {
      ...
      "return a profile if the user (root user) is known" in {
        val actual = UnsafeZioRun.runOrThrow(
          UsersRestService(_.getUserByEmail(KnoraSystemInstances.Users.SystemUser, Email.unsafeFrom(rootUser.email))),
        )
    ...
    "calling getUserByUsername" should {
      ...
      def getUserByUsername(requestingUser: User, username: Username) =
        UsersRestService(_.getUserByUsername(requestingUser, username))
    ...
    "asked to update the user's project membership" should {
      ...
      "ADD user to project" in {
        ...
        val membershipsBeforeUpdate =
          UnsafeZioRun.runOrThrow(UsersRestService(_.getProjectMemberShipsByUserIri(normalUser.userIri)))
        membershipsBeforeUpdate.projects should equal(Chunk.empty)

        // add user to images project (00FF)
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.addUserToProject(rootUser, normalUser.userIri, imagesProject.projectIri)),
        )
      ...
      "ADD user to project as project admin" in {
        ...
        // add user to images project (00FF)
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.addUserToProject(rootUser, normalUser.userIri, incunabulaProject.projectIri)),
        )
      ...
      "DELETE user from project and also as project admin" in {
        ...
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.addUserToProjectAsAdmin(rootUser, normalUser.userIri, imagesProject.projectIri)),
        )

        // verify that the user has been added as project admin to the images project
        val projectAdminMembershipsBeforeUpdate =
          UnsafeZioRun.runOrThrow(UsersRestService(_.getProjectAdminMemberShipsByUserIri(normalUser.userIri)))
        ...
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.removeUserFromProject(rootUser, normalUser.userIri, imagesProject.projectIri)),
        )
    ...
    "asked to update the user's project admin group membership" should {
      ...
      "DELETE user from project admin group" in {
        ...
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.removeUserFromProjectAsAdmin(rootUser, normalUser.userIri, imagesProject.projectIri)),
        )
    ...
    "asked to update the user's group membership" should {
      ...
      "ADD user to group" in {
        ...
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.addUserToGroup(rootUser, normalUser.userIri, imagesReviewerGroup.groupIri)),
        )
      ...
      "DELETE user from group" in {
        ...
        UnsafeZioRun.runOrThrow(
          UsersRestService(_.removeUserFromGroup(rootUser, normalUser.userIri, imagesReviewerGroup.groupIri)),
        )
