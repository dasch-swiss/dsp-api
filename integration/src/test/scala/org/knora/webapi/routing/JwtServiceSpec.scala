  ...
  "The JWTHelper" should {
    ...
    "create a token" in {
      val runZio = for {
        token <-
          ZIO
            .serviceWithZIO[JwtService](_.createJwt(SharedTestDataADM.anythingUser1, Map("foo" -> JsString("bar"))))
            .map(_.jwtString)
        useriri <- ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(token))
      } yield useriri
    ...
    "validate a token" in {
      val tokenValid = ZIO.serviceWithZIO[JwtService](_.validateToken(validToken))
      UnsafeZioRun.runOrThrow(tokenValid) should be(true)
    }

    "extract the user's IRI" in {
      val runZio = ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(validToken))
      UnsafeZioRun.runOrThrow(runZio) should be(Some(SharedTestDataADM.anythingUser1.id))
    }
    ...
    "not decode an invalid token" in {
      ...
      val runZio = ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(invalidToken))
    ...
    "not decode a token with an invalid user IRI" in {
      ...
      val runZio = ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(tokenWithInvalidSubject))
      UnsafeZioRun.runOrThrow(runZio) should be(None)
    }
    ...
    "not decode a token with missing required content" in {
      ...
      val runZio = ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(tokenWithMissingExp))
      UnsafeZioRun.runOrThrow(runZio) should be(None)
    }
    ...
    "not decode an expired token" in {
      ...
      val runZio = ZIO.serviceWithZIO[JwtService](_.extractUserIriFromToken(expiredToken))
      UnsafeZioRun.runOrThrow(runZio) should be(None)
    }
    ...
    "reject a token with a different issuer than the one who created the token" in {
      ...
      val runZio = ZIO.serviceWithZIO[JwtService](_.validateToken(tokenWithDifferentIssuer))
