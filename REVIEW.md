...
- [ ] No string concatenation in SPARQL — new queries use the `sparql"..."` interpolator from `modules/sparql-builder/`; no **new** rdf4j SparqlBuilder usage (existing files keep it until migrated) — see `docs/development/dsp-api-sparql-queries.md`; the admin SPARQL passthrough is the one deliberate exception, see `CONVENTIONS.md` § SPARQL
- [ ] A migrated query site's rendered SPARQL was diffed against the old builder's `getQueryString` output
