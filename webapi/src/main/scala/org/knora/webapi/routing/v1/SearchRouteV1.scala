...
object SearchRouteV1 extends Authenticator {
    ...
    def makeExtendedSearchRequestMessage(userProfile: UserProfileV1, reverseParams: Map[String, Seq[String]]): ExtendedSearchGetRequestV1 = {
        ...
        val restypeIri: Option[IRI] = params.get("filter_by_restype") match {
            case Some(List(restype: IRI)) => Some(stringFormatter.toIri(restype, () => throw BadRequestException(s"Value for param 'filter_by_restype' for extended search $restype is not a valid IRI. Please make sure that it was correctly URL encoded.")))
            case other => None
        }

        // only one value is expected
        val projectIri: Option[IRI] = params.get("filter_by_project") match {
            case Some(List(project: IRI)) => Some(stringFormatter.toIri(project, () => throw BadRequestException(s"Value for param 'filter_by_project' for extended search $project is not a valid IRI. Please make sure that it was correctly URL encoded.")))
            case other => None
        }

        // only one value is expected
        val ownerIri: Option[IRI] = params.get("filter_by_owner") match {
            case Some(List(owner: IRI)) => Some(stringFormatter.toIri(owner, () => throw BadRequestException(s"Value for param 'filter_by_owner' for extended search $owner is not a valid IRI. Please make sure that it was correctly URL encoded.")))
            case other => None
        }
        ...
        val propertyIri: Seq[IRI] = params.get("property_id") match {
            case Some(propertyList: Seq[IRI]) => propertyList.map(
                prop => stringFormatter.toIri(prop, () => throw BadRequestException(s"Value for param 'property_id' for extended search $prop is not a valid IRI. Please make sure that it was correctly URL encoded."))
            )
            case other => Nil
        }
        ...
        val showNRows: Int = params.get("show_nrows") match {
            case Some(showNRowsStrList: Seq[String]) =>
                val showNRowsVal = stringFormatter.toInt(showNRowsStrList.head, () => throw BadRequestException(s"Can't parse integer parameter 'show_nrows' for extended search: $showNRowsStrList"))
                showNRowsVal match {
        ...
        val startAt: Int = params.get("start_at") match {
            case Some(startAtStrList: Seq[String]) =>
                stringFormatter.toInt(startAtStrList.head, () => throw BadRequestException(s"Can't parse integer parameter 'start_at' for extended search: $startAtStrList"))
            case None => 0
        }
    ...
    def makeFulltextSearchRequestMessage(userProfile: UserProfileV1, searchval: String, params: Map[String, String]): FulltextSearchGetRequestV1 = {
        ...
        val restypeIri: Option[IRI] = params.get("filter_by_restype") match {
            case Some(restype: IRI) => Some(stringFormatter.toIri(restype, () => throw BadRequestException(s"Unexpected param 'filter_by_restype' for extended search: $restype")))
            case other => None
        }
        val projectIri: Option[IRI] = params.get("filter_by_project") match {
            case Some(project: IRI) => Some(stringFormatter.toIri(project, () => throw BadRequestException(s"Unexpected param 'filter_by_project' for extended search: $project")))
            case other => None
        }
        ...
        val showNRows: Int = params.get("show_nrows") match {
            case Some(showNRowsStr) =>
                val showNRowsVal = stringFormatter.toInt(showNRowsStr, () => throw BadRequestException(s"Can't parse integer parameter 'show_nrows' for extended search: $showNRowsStr"))
                showNRowsVal match {
                    case -1 => defaultShowNRows
                    case _ => showNRowsVal
                }
        ...
        val startAt: Int = params.get("start_at") match {
            case Some(startAtStr) =>
                stringFormatter.toInt(startAtStr, () => throw BadRequestException(s"Can't parse integer parameter 'start_at' for extended search: $startAtStr"))
            case None => 0
        }
