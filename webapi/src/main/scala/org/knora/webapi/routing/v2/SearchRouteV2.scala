...
class SearchRouteV2(routeData: KnoraRouteData) extends KnoraRoute(routeData) with Authenticator {
  ...
  private def fullTextSearchCount(featureFactoryConfig: FeatureFactoryConfig): Route =
    path("v2" / "search" / "count" / Segment) { searchStr => // TODO: if a space is encoded as a "+", this is not converted back to a space
      get { requestContext =>
        ...
        val escapedSearchStr =
          stringFormatter.toSparqlEncodedString(searchStr,
  ...
  private def gravsearchGet(featureFactoryConfig: FeatureFactoryConfig): Route = path("v2" / "searchextended" / Segment) {
      ...
      get { requestContext =>
        {
          ...
          val searchString =
            stringFormatter.toSparqlEncodedString(searchval,
  ...
  private def searchByLabel(featureFactoryConfig: FeatureFactoryConfig): Route = path("v2" / "searchbylabel" / Segment) {
      ...
      get { requestContext =>
        {
          ...
          val requestMessage: Future[SearchResourceByLabelRequestV2] = for {
            ...
            SearchResourceByLabelRequestV2(
              searchValue = searchString,
              offset = offset,
              limitToProject = limitToProject,
              limitToResourceClass = limitToResourceClass,
