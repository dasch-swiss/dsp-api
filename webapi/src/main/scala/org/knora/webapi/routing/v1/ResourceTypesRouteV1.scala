        ...
        path("v1" / "resourcetypes" / Segment) { iri =>
            get {
                requestContext =>
                    ...
                    val resourceTypeIri = stringFormatter.validateIri(iri, () => throw BadRequestException(s"Invalid resource class IRI: $iri"))
            ...
            get {
                requestContext =>
                    ...
                    val namedGraphIri = vocabularyId match {
                        ...
                        case other => Some(stringFormatter.validateIri(vocabularyId, () => throw BadRequestException(s"Invalid vocabulary IRI: $vocabularyId")))
                    }
            ...
            get {
                requestContext =>
                    ...
                    val requestMessage = vocabularyId match {
                        ...
                        case Some(vocId) =>
                            val namedGraphIri = stringFormatter.validateIri(vocId, () => throw BadRequestException(s"Invalid vocabulary IRI: $vocabularyId"))
                        ...
                        case None => // no vocabulary id given, check for restype
                            resourcetypeId match {
                                case Some(restypeId) => // get property types for given resource type
                                    val resourceClassIri = stringFormatter.validateIri(restypeId, () => throw BadRequestException(s"Invalid vocabulary IRI: $restypeId"))
            ...
            iri =>
                get {
                    requestContext =>
                        ...
                        // TODO: Check that this is the IRI of a resource type and not just any IRI
                        val resourceClassIri = stringFormatter.validateIri(iri, () => throw BadRequestException(s"Invalid resource class IRI: $iri"))
