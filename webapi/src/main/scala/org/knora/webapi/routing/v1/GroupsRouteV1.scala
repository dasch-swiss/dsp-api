                        ...
                        val requestMessage = if (identifier != "iri") { // identify group by groupname/projectIri
                            maybeProjectIri match {
                                case Some(projectIri) => {
                                    ...
                                    val ckeckedProjectIri = stringFormatter.validateIri(projectIri, () => throw BadRequestException(s"Invalid project IRI $projectIri"))
                            ...
                            val checkedGroupIri = stringFormatter.validateIri(value, () => throw BadRequestException(s"Invalid group IRI $value"))
