...
class NewListsRouteADMFeature(routeData: KnoraRouteData) extends KnoraRoute(routeData)
    ...
    override def knoraApiPath: Route = getLists ~ createList ~ getListNode ~ updateList ~ createListChildNode ~ deleteListNode ~
    ...
    private def oldGetList: Route = path(ListsBasePath / Segment) { iri =>
        get {
