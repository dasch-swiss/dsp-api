...
import org.knora.webapi.messages.v1.responder.standoffmessages.RepresentationV1JsonProtocol.{createStandoffApiRequestV1Format, changeStandoffApiRequestV1Format}
            ...
            post {
                entity(as[String]) { xml: String =>
                    requestContext =>
                        ...
                        val userProfile = getUserProfileV1(requestContext)
                        val requestMessage = CreateMappingRequestV1(xml = xml, label = "this is my mapping", projectIri = projectIri, mappingName = "myMapping", userProfile = userProfile, UUID.randomUUID())
