                    ...
                    (InputValidation.toIri(propIri, () => throw BadRequestException(s"Invalid property IRI $propIri")), values.map {
                        case (givenValue: CreateResourceValueV1) =>
                            ...
                            givenValue match {
                                ...
                                case CreateResourceValueV1(Some(richtext: CreateRichtextV1), _, _, _, _, _, _, _, _, _, _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, Some(linkValue: IRI), _, _, _, _, _, _, _, _, _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, _, Some(intValue: Int), _, _, _, _, _, _, _, _, _, _, comment) => CreateValueV1WithComment(IntegerValueV1(intValue), comment)

                                case CreateResourceValueV1(_, _, _, Some(decimalValue: BigDecimal), _, _, _, _, _, _, _, _, _, comment) =>
                                    CreateValueV1WithComment(DecimalValueV1(decimalValue), comment)

                                case CreateResourceValueV1(_, _, _, _, Some(booleanValue: Boolean), _, _, _, _, _, _, _, _, comment) =>
                                    CreateValueV1WithComment(BooleanValueV1(booleanValue), comment)

                                case CreateResourceValueV1(_, _, _, _, _, Some(uriValue: String), _, _, _, _, _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, _, _, _, _, _, Some(dateStr: String), _, _, _, _, _, _, comment) =>
                                    CreateValueV1WithComment(DateUtilV1.createJDCValueV1FromDateString(dateStr), comment)

                                case CreateResourceValueV1(_, _, _, _, _, _, _, Some(colorStr: String), _, _, _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, _, _, _, _, _, _, _, Some(geomStr: String), _, _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, _, _, _, _, _, _, _, _, Some(hlistValue: IRI), _, _, _, comment) =>
                                ...
                                case CreateResourceValueV1(_, _, _, _, _, _, _, _, _, _, Some(timeValue: BigDecimal), _, _, comment) =>
                                    CreateValueV1WithComment(TimeValueV1(timeValue), comment)

                                case CreateResourceValueV1(_, _, _, _, _, _, _, _, _, _, _, Some(Seq(timeval1: BigDecimal, timeval2: BigDecimal)), _, comment) =>
                                    CreateValueV1WithComment(IntervalValueV1(timeval1, timeval2), comment)

                                case CreateResourceValueV1(_, _, _, _, _, _, _, _, _, _, _, _, Some(geonameStr: String), comment) =>
