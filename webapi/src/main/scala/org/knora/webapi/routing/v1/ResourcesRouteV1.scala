...
final case class ResourcesRouteV1()(
  ...
  def makeRoute: Route = {
    ...
    def toPropertyIriFromOtherOntologyInXmlImport(
          ...
          for {
            ...
            propertiesWithValues <- ZIO.foreach(propertyElements) { propertyNode =>
                                      for {
                                        ...
                                        value <-
                                          if (
                                            valueNodes.size == 1 && valueNodes.head.attribute("knoraType").isDefined
                                          ) { knoraDataTypeXml(valueNodes.head) }
                                          else { knoraDataTypeXml(propertyNode) }
     ...
     */
    def knoraDataTypeXml(node: Node): ZIO[StringFormatter, Throwable, CreateResourceValueV1] = {
