  ...
  override def makeRoute(featureFactoryConfig: FeatureFactoryConfig): Route = {
    ...
    def makeCreateResourceRequestMessage(
      ...
      for {
        ...
        valuesToBeCreated: Map[IRI, Seq[CreateValueV1WithComment]] <-
          ActorUtil.sequenceFutureSeqsInMap(valuesToBeCreatedWithFuture)
    ...
    def getNamedGraphInfos(mainOntologyIri: IRI, userProfile: UserADM): Future[Map[IRI, NamedGraphEntityInfoV1]] = {
      ...
      def getNamedGraphInfosRec(
        ...
        for {
          ...
          initialNamedGraphInfo: NamedGraphEntityInfoV1 <-
            (responderManager ? NamedGraphEntityInfoRequestV1(initialOntologyIri, userProfile))
    ...
    def generateSchemasFromOntologies(
      ...
      for {
        ...
        namedGraphInfos: Map[IRI, NamedGraphEntityInfoV1] <-
          getNamedGraphInfos(mainOntologyIri = internalOntologyIri, userProfile = userProfile)
        ...
        entityInfoResponses: immutable.Iterable[(IRI, EntityInfoGetResponseV1)] <-
          Future.sequence(entityInfoResponseFutures)
        ...
        knoraXmlImportSchemaXml: String =
          FileUtil.readTextResource(OntologyConstants.KnoraXmlImportV1.KnoraXmlImportNamespacePrefixLabel + ".xsd")
    ...
    path("v1" / "resources") {
      get {
        ...
        requestContext =>
          val requestMessage = for {
            ...
            limitOfResults =
              stringFormatter.validateInt(limit, throw BadRequestException(s"Invalid param limit: $limit"))
      ...
      get { requestContext =>
        val requestMessage = for {
          ...
          resIri =
            stringFormatter.validateAndEscapeIri(iri, throw BadRequestException(s"Invalid param resource IRI: $iri"))
      ...
      post {
        entity(as[String]) { xml => requestContext =>
          val requestMessage = for {
            ...
            resourcesToCreate: Seq[CreateResourceFromXmlImportRequestV1] =
              importXmlToCreateResourceRequests(rootElement)
