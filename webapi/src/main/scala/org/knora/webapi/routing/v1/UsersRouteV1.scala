    ...
    def knoraApiPath(_system: ActorSystem, settings: SettingsImpl, log: LoggingAdapter): Route = {
        ...
        path("v1" / "users") {
            get {
                ...
                post {
                    /* create a new user */
                    entity(as[CreateUserApiRequestV1]) { apiRequest =>
                        requestContext =>
                            val userProfile = getUserProfileV1(requestContext)
                    ...
                    parameters("identifier" ? "iri") { (identifier: String) =>
                        requestContext =>
                            ...
                            val requestMessage = if (identifier == "email") {
                                ...
                                val userIri = stringFormatter.validateIri(value, () => throw BadRequestException(s"Invalid user IRI $value"))
            ...
            path("v1" / "users" / "projects" / Segment) { userIri =>
                get {
                    ...
                    requestContext =>
                        ...
                        val checkedUserIri = stringFormatter.validateIri(userIri, () => throw BadRequestException(s"Invalid user IRI $userIri"))
            ...
            path("v1" / "users" / "projects" / Segment / Segment) { (userIri, projectIri) =>
                post {
                    ...
                    requestContext =>
                        ...
                        val checkedUserIri = stringFormatter.validateIri(userIri, () => throw BadRequestException(s"Invalid user IRI $userIri"))
                        val checkedProjectIri = stringFormatter.validateIri(projectIri, () => throw BadRequestException(s"Invalid project IRI $projectIri"))
            ...
            path("v1" / "users" / "groups" / Segment / Segment) { (userIri, groupIri) =>
                post {
                    ...
                    requestContext =>
                        ...
                        val checkedUserIri = stringFormatter.validateIri(userIri, () => throw BadRequestException(s"Invalid user IRI $userIri"))
                        val checkedGroupIri = stringFormatter.validateIri(groupIri, () => throw BadRequestException(s"Invalid group IRI $groupIri"))
