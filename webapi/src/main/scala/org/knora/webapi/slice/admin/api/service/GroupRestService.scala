...
final case class GroupRestService(
  ...
  def getGroups(ignored: Unit): Task[GroupsGetResponseADM] = for {
    internal <- groupService.findAllRegularGroups
    external <- format.toExternal(GroupsGetResponseADM(internal))
  } yield external

  def getGroupByIri(iri: GroupIri): Task[GroupGetResponseADM] = for {
  ...
  def getGroupMembers(user: User)(iri: GroupIri): Task[GroupMembersGetResponseADM] = for {
  ...
  def postGroup(user: User)(request: GroupCreateRequest): Task[GroupGetResponseADM] = for {
  ...
  def putGroup(user: User)(iri: GroupIri, request: GroupUpdateRequest): Task[GroupGetResponseADM] = for {
    _ <- auth.ensureSystemAdminOrProjectAdminOfGroup(user, iri)
    _ <- ZIO
  ...
  def putGroupStatus(user: User)(iri: GroupIri, request: GroupStatusUpdateRequest): Task[GroupGetResponseADM] =
    updateStatus(user)(iri, request.status)

  def deleteGroup(user: User)(iri: GroupIri): Task[GroupGetResponseADM] =
    updateStatus(user)(iri, GroupStatus.inactive)

  private def updateStatus(user: User)(iri: GroupIri, status: GroupStatus) = for {
