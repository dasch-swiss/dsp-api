...
object ChangeOntologyMetadataQuery extends QueryBuilderHelper {
  ...
  def build(
    ...
    val deletePatterns: List[TriplePattern] = {
      val labelDelete =
        if (newLabel.nonEmpty) List(ontology.has(RDFS.LABEL, oldLabel))
        else Nil

      val commentDelete =
        if (hasOldComment && (deleteOldComment || newComment.nonEmpty))
          List(ontology.has(RDFS.COMMENT, oldComment))
        else Nil
    ...
    val wherePatterns: List[TriplePattern] = {
      ...
      val commentPattern =
        if (hasOldComment) List(ontology.has(RDFS.COMMENT, oldComment))
        else Nil
  ...
  private def buildInsertPatterns(
    ...
    val labelPattern =
      newLabel.map(label => ontology.has(RDFS.LABEL, toRdfLiteral(label))).toList

    val commentPattern =
      newComment.map(comment => ontology.has(RDFS.COMMENT, toRdfLiteral(comment.value))).toList
