...
object DeleteLinkQuery extends QueryBuilderHelper {
  ...
  def build(
    ...
    if (!linkUpdate.deleteDirectLink) {
      throw SparqlGenerationException("linkUpdate.deleteDirectLink must be true in this SPARQL template")
    }
    if (!linkUpdate.linkValueExists) {
      throw SparqlGenerationException("linkUpdate.linkValueExists must be true in this SPARQL template")
    }
    if (!linkUpdate.directLinkExists) {
      throw SparqlGenerationException("linkUpdate.directLinkExists must be true in this SPARQL template")
    }
    if (linkUpdate.newReferenceCount != 0) {
      throw SparqlGenerationException("linkUpdate.newReferenceCount must be 0 in this SPARQL template")
    }
