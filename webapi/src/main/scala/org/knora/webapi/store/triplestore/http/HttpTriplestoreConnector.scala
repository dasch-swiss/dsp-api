...
class HttpTriplestoreConnector extends Actor with ActorLogging with InstrumentationSupport {
  ...
  private def sparqlHttpConstruct(sparqlConstructRequest: SparqlConstructRequest): Try[SparqlConstructResponse] = {
    ...
    def parseTurtleResponse(sparql: String,
                            turtleStr: String,
                            rdfFormatUtil: RdfFormatUtil): Try[SparqlConstructResponse] = {
      val parseTry = Try {
    ...
    for {
      turtleStr <- getSparqlHttpResponse(sparqlConstructRequest.sparql,
                                         isUpdate = false,
  ...
  private def sparqlHttpExtendedConstruct(
      sparqlExtendedConstructRequest: SparqlExtendedConstructRequest): Try[SparqlExtendedConstructResponse] = {
    // println(sparql)
    val rdfFormatUtil: RdfFormatUtil =
      RdfFeatureFactory.getRdfFormatUtil(sparqlExtendedConstructRequest.featureFactoryConfig)

    val parseTry = for {
      turtleStr <- getSparqlHttpResponse(sparqlExtendedConstructRequest.sparql,
                                         isUpdate = false,
                                         acceptMimeType = mimeTypeTextTurtle)
  ...
  private def resetTripleStoreContent(rdfDataObjects: Seq[RdfDataObject],
                                      prependDefaults: Boolean = true): Try[ResetRepositoryContentACK] = {
  ...
  /**
    ...
    */
  private def insertDataIntoTriplestore(rdfDataObjects: Seq[RdfDataObject],
                                        prependDefaults: Boolean = true): Try[InsertTriplestoreContentACK] = {
  ...
  private def checkFusekiTriplestore(afterAutoInit: Boolean = false): Try[CheckTriplestoreResponse] = {
    ...
    try {
      ...
      if (neededDataset.nonEmpty) {
        ...
        Success(
          CheckTriplestoreResponse(triplestoreStatus = TriplestoreStatus.ServiceAvailable,
        ...
        if (settings.triplestoreAutoInit) {
          ...
          Success(
            CheckTriplestoreResponse(triplestoreStatus = TriplestoreStatus.NotInitialized,
      ...
      case e: Exception =>
        ...
        Success(
          CheckTriplestoreResponse(triplestoreStatus = TriplestoreStatus.ServiceUnavailable,
  ...
  def returnInsertGraphDataResponse(graphName: String)(
