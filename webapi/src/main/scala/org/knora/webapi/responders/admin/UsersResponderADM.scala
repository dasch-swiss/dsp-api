...
class UsersResponderADM(responderData: ResponderData) extends Responder(responderData) with InstrumentationSupport {
  ...
  def receive(msg: UsersResponderRequestADM) = msg match {
    ...
    case UserChangeBasicUserInformationRequestADM(
          userIri,
          changeUserRequest,
    ...
    def changeBasicUserDataTask(
      ...
      for {
        ...
        usernameTaken: Boolean <-
          userByUsernameExists(changeUserRequest.username, Some(currentUserInformation.get.username))
        _ = if (usernameTaken) {
              throw DuplicateValueException(
                s"User with the username '${changeUserRequest.username.get}' already exists"
              )
            }
  ...
  private def userGroupMembershipRemoveRequestADM(
    ...
    def userGroupMembershipRemoveRequestTask(
      ...
      for {
        ...
        _ =
          log.debug("userGroupMembershipRemoveRequestADM - currentGroupMembershipIris: {}", currentGroupMembershipIris)
        ...
        _ =
          log.debug("userGroupMembershipRemoveRequestADM - updatedGroupMembershipIris: {}", updatedGroupMembershipIris)
    ...
    for {
      ...
      _ = if (userUpdatePayload.password.isDefined) {
            if (updatedUserADM.password != userUpdatePayload.password)
              throw UpdateNotPerformedException(
                "User's 'password' was not updated. Please report this as a possible bug."
              )
          }
        ...
        createNewUserSparqlString = org.knora.webapi.messages.twirl.queries.sparql.admin.txt
                                      .createNewUser(
                                        ...
                                        username = stringFormatter.validateAndEscapeUsername(
                                          createRequest.username,
                                          throw BadRequestException(
                                            s"The username '${createRequest.username}' contains invalid characters"
                                          )
                                        ),
