...
class SearchResponderV2 extends ResponderWithStandoffV2 {
    ...
    abstract class AbstractSparqlTransformer(typeInspectionResult: GravsearchTypeInspectionResult, querySchema: ApiV2Schema) extends WhereTransformer {
        ...
        protected def convertStatementForPropertyType(inputOrderBy: Seq[OrderCriterion])(propertyTypeInfo: PropertyTypeInfo, statementPattern: StatementPattern, typeInspectioResult: GravsearchTypeInspectionResult): Seq[QueryPattern] = {
            ...
            val subjectIsResource: Boolean = typeInspectionResult.getTypeOfEntity(statementPattern.subj) match {
                case Some(NonPropertyTypeInfo(subjectTypeIri)) if OntologyConstants.KnoraApi.isKnoraApiV2Resource(subjectTypeIri) => true
                case _ => false
            }
                        ...
                        typeInfo match {
                            ...
                            case nonPropInfo: NonPropertyTypeInfo =>
                                ...
                                nonPropInfo.typeIri.toString match {
                                    ...
                                    case _ => throw GravsearchException(s"${regexFunctionCall.textVar} is expected to be of type xsd:string")
                                }

                            case _ => throw GravsearchException(s"${regexFunctionCall.textVar} is expected to be of type NonPropertyTypeInfo")
                        }

                    case None =>
                        throw GravsearchException(s"type information about ${regexFunctionCall.textVar} is missing")
                }
        ...
        private def handleKnoraFunctionCall(functionCallExpression: FunctionCallExpression, typeInspectionResult: GravsearchTypeInspectionResult, isTopLevel: Boolean): TransformedFilterPattern = {
            ...
            functionIri.toString match {
                ...
                case OntologyConstants.KnoraApiV2WithValueObjects.MatchInStandoff =>
                    if (querySchema == ApiV2Simple) {
                        throw GravsearchException(s"Function $functionIri cannot be used in a Gravsearch query written in the simple schema")
                    }
                    ...
                    if (functionCallExpression.args.size != 3) throw GravsearchException(s"Three arguments are expected for $functionIri")
