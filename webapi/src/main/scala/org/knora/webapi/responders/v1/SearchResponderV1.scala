...
class SearchResponderV1(responderData: ResponderData) extends Responder(responderData) {
  ...
  private def fulltextSearchV1(searchGetRequest: FulltextSearchGetRequestV1): Future[SearchGetResponseV1] = {
    ...
    for {
      ...
      subjects: Vector[SearchResultRowV1] =
        groupedByResourceIri
          .foldLeft(Vector.empty[SearchResultRowV1]) { case (subjectsAcc, (resourceIri, rows)) =>
  ...
  private def extendedSearchV1(searchGetRequest: ExtendedSearchGetRequestV1): Future[SearchGetResponseV1] = {
    ...
    for {
      ...
      searchCriteria: Seq[SearchCriterion] =
        (searchGetRequest.propertyIri, searchGetRequest.compareProps, searchGetRequest.searchValue).zipped.map {
                ...
                case OntologyConstants.KnoraBase.DateValue =>
                  ...
                  val datestring =
                    stringFormatter.validateDate(
                      searchval,
                      throw BadRequestException(s"Invalid date format: $searchval")
                    )
  ...
  private def findMaxPreviewDimensions(subjects: Seq[SearchResultRowV1]): (Int, Int) =
    subjects.foldLeft((0, 0)) { case ((accX, accY), searchResultRow) =>
   ...
   */
  private case class MatchingValue(
    valueTypeIri: IRI,
    propertyIri: IRI,
