                                            ...
                                            group = GroupADM(
                                                      ...
                                                      description = propsMap
                                                        .getOrElse(
                                                          ...
                                                          throw InconsistentRepositoryDataException(
                                                            s"Group $groupIri has no description attached"
                                                          )
                                                        )
                                                        .head
                                                        .asInstanceOf[StringLiteralV2]
  ...
  private def createGroupADM(
    ...
    def createGroupTask(
      ...
      for {
        ...
        _ =
          if (
            !requestingUser.permissions
              .isProjectAdmin(createRequest.project) && !requestingUser.permissions.isSystemAdmin
          ) {
            // not a project admin and not a system admin
        ...
        maybeProjectADM: Option[ProjectADM] <- (responderManager ? ProjectGetADM(
                                                 identifier =
                                                   ProjectIdentifierADM(maybeIri = Some(createRequest.project)),
                                                 featureFactoryConfig = featureFactoryConfig,
                                                 requestingUser = KnoraSystemInstances.Users.SystemUser
                                               )).mapTo[Option[ProjectADM]]
        ...
        groupIri: IRI <-
          checkOrCreateEntityIri(customGroupIri, stringFormatter.makeRandomGroupIri(projectADM.shortcode))
  ...
  private def changeGroupBasicInformationRequestADM(
    ...
    def changeGroupTask(
      ...
      for {
        ...
        groupADM: GroupADM =
          maybeGroupADM.getOrElse(throw NotFoundException(s"Group <$groupIri> not found. Aborting update request."))
    ...
    for {
      ...
      _ = if (groupUpdatePayload.status.isDefined) {
            if (updatedGroup.status != groupUpdatePayload.status.get)
              throw UpdateNotPerformedException(
                "Group's 'status' was not updated. Please report this as a possible bug."
              )
          }
