...
import org.knora.webapi.messages.admin.responder.usersmessages.{UserADM, UsersGetResponseADM}
import org.knora.webapi.messages.store.triplestoremessages._
import org.knora.webapi.messages.v1.responder.groupmessages._
import org.knora.webapi.messages.v1.responder.projectmessages._
import org.knora.webapi.messages.v1.responder.usermessages.{UserProfileV1, UsersGetResponseV1}
...
class GroupsResponderADM extends Responder with GroupsADMJsonProtocol {
    ...
    def receive = {
        ...
        case GroupGetRequestADM(iri, userProfile) => future2Message(sender(), groupInfoByIRIGetRequestV1(iri, userProfile), log)
    ...
    private def groupsGetADM(requestingUser: UserADM): Future[Seq[GroupADM]] = {
        ...
        for {
            ...
            groups: Seq[GroupADM] = statements.map {
                case (groupIri: IRI, propsMap: Map[String, LiteralV2]) =>

                    val projectIri = propsMap.getOrElse(OntologyConstants.KnoraBase.BelongsToProject, throw InconsistentTriplestoreDataException(s"Group $groupIri has no project attached"))
                    val project = (responderManager <- ProjectGetADM(maybeIri = Some(projectIri))
