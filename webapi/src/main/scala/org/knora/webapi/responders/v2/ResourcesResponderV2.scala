  ...
  def extractEventsFromHistory(
    ...
    for {
      ...
      resourceCreationEvent: Seq[ResourceAndValueHistoryEvent] =
        getResourceCreationEvent(resourceAtCreation, creationTimeHist)
    ...
    def makeTaskFuture(resourceIri: IRI): Future[ReadResourcesSequenceV2] = {
      for {
        ...
        internalCreateResource: CreateResourceV2 <-
          Future(createResourceRequestV2.createResource.toOntologySchema(InternalSchema))
        ...
        resourceClassInfo: ReadClassInfoV2 =
          resourceClassEntityInfoResponse.classInfoMap(internalCreateResource.resourceClassIri)
      ...
      _ =
        if (
          !createResourceRequestV2.requestingUser.permissions
            .hasPermissionFor(ResourceCreateOperation(internalResourceClassIri.toString), projectIri, None)
        ) {
  ...
  private def markResourceAsDeletedV2(deleteResourceV2: DeleteOrEraseResourceRequestV2): Future[SuccessResponseV2] = {
    def makeTaskFuture: Future[SuccessResponseV2] =
      for {
        ...
        _ =
          if (
            deleteResourceV2.maybeDeleteDate
              .exists(!_.isAfter(resource.lastModificationDate.getOrElse(resource.creationDate)))
          ) {
            throw BadRequestException(
  ...
  private def verifyResource(
    ...
    val resourceFuture: Future[ReadResourcesSequenceV2] = for {
      ...
      unescapedLabel: String =
        stringFormatter.fromSparqlEncodedString(resourceReadyToCreate.sparqlTemplateResourceToCreate.resourceLabel)
      ...
      for {
        ...
        recursiveResults: GraphQueryResults <- if (rows.isEmpty) {
                                                 ...
                                                 val edges: Set[QueryResultEdge] = rows.map { row: VariableResultsRow =>
                                                   ...
                                                   // nodes.
                                                   val hasPermission: Boolean = visibleNodeIris.contains(
                                                     edge.sourceNodeIri
                                                 ...
                                                 if (depth == 1) {
                                                   ...
                                                   val traversedEdgesForRecursion: Set[QueryResultEdge] =
