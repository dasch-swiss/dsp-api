...
class ProjectsResponderV1(responderData: ResponderData) extends Responder(responderData) {
    ...
    for {
      ...
      projectsWithProperties: Map[IRI, Map[IRI, Seq[String]]] = projectsResponseRows.groupBy(_.rowMap("s")).map {
        case (projIri: String, rows: Seq[VariableResultsRow]) =>
          (projIri, rows.groupBy(_.rowMap("p")).map {
      ...
      projects = projectsWithProperties.map {
        case (projectIri: String, propsMap: Map[String, Seq[String]]) =>
          ...
          ProjectInfoV1(
            id = projectIri,
            shortname = propsMap
              .getOrElse(OntologyConstants.KnoraAdmin.ProjectShortname,
                         throw InconsistentRepositoryDataException(s"Project: $projectIri has no shortname defined."))
              .head,
            shortcode = propsMap
              .getOrElse(OntologyConstants.KnoraAdmin.ProjectShortcode,
            ...
            status = propsMap
              .getOrElse(OntologyConstants.KnoraAdmin.Status,
  ...
  private def getOntologiesForProjects(projectIris: Set[IRI] = Set.empty[IRI],
    ...
    for {
      ...
      namedGraphsResponse.vocabularies
        .map { namedGraph: NamedGraphV1 =>
  ...
  /**
    ...
    */
  private def createProjectInfoV1(projectResponse: Seq[VariableResultsRow],
                                  projectIri: IRI,
                                  ontologies: Seq[IRI],
                                  userProfile: Option[UserProfileV1]): ProjectInfoV1 = {
