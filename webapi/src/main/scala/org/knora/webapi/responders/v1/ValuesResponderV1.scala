...
class ValuesResponderV1(responderData: ResponderData) extends Responder(responderData) {
  ...
  private def createValueV1(createValueRequest: CreateValueRequestV1): Future[CreateValueResponseV1] = {
    ...
    def makeTaskFuture(userIri: IRI): Future[CreateValueResponseV1] =
      for {
        ...
        cardinalityOK =
          resourceFullResponse.props.flatMap(_.properties.find(_.pid == createValueRequest.propertyIri)) match {
  ...
  private def verifyMultipleValueCreation(
    ...
    for {
      valueVerificationResponses: Map[IRI, Seq[CreateValueResponseV1]] <-
        ActorUtil.sequenceFutureSeqsInMap(valueVerificationFutures)
  ...
  private def getValueVersionHistoryResponseV1(
    ...
    for {
      ...
      versionRowsVector =
        versionMap2Vector(versionMap, versionHistoryRequest.currentValueIri, Vector.empty[Map[String, String]])
  ...
  private def sparqlQueryResults2ValueQueryResult(
    ...
    if (rows.nonEmpty) {
      ...
      for {
        ...
        creationDate =
          getValuePredicateObject(predicateIri = OntologyConstants.KnoraBase.ValueCreationDate, rows = rows)
