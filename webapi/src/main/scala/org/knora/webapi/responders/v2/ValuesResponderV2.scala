...
class ValuesResponderV2(responderData: ResponderData) extends Responder(responderData) {
    ...
    def makeTaskFuture: Future[CreateValueResponseV2] = {
      for {
        ...
        submittedInternalPropertyIri: SmartIri <-
          Future(createValueRequest.createValue.propertyIri.toOntologySchema(InternalSchema))
        ...
        cardinalityInfo: Cardinality.KnoraCardinalityInfo = classInfo.allCardinalities.getOrElse(
                                                              ...
                                                              throw BadRequestException(
                                                                s"Resource <${createValueRequest.createValue.resourceIri}> belongs to class <${resourceInfo.resourceClassIri.toOntologySchema(ApiV2Complex)}>, which has no cardinality for property <${createValueRequest.createValue.propertyIri}>"
                                                              )
                                                            )
  ...
  private def generateSparqlToCreateMultipleValuesV2(
    ...
    for {
      ...
      sparqlForPropertyValues: Map[SmartIri, Seq[InsertSparqlWithUnverifiedValue]] <-
        ActorUtil.sequenceSeqFuturesInMap(sparqlForPropertyValueFutures)
  ...
  private def generateInsertSparqlWithUnverifiedValue(
    ...
    for {
      // Make new value UUID.
      newValueUUID: UUID <-
        Future.successful(makeNewValueUUID(valueToCreate.customValueIri, valueToCreate.customValueUUID))
                               ...
                               case otherValueContentV2 =>
                                 ...
                                 org.knora.webapi.messages.twirl.queries.sparql.v2.txt
                                   .generateInsertStatementsForCreateValue(
                                     ...
                                     newValueUUID = newValueUUID,
                                     linkUpdates =
                                       Seq.empty[SparqlTemplateLinkUpdate], // This is empty because we have to generate SPARQL for standoff links separately.
    ...
    def makeTaskFutureToUpdateValueContent(
      ...
      for {
        ...
        submittedInternalValueContent: ValueContentV2 =
          updateValueContentV2.valueContent.toOntologySchema(InternalSchema)
  ...
  private def updateOrdinaryValueV2AfterChecks(
    ...
    for {
      newValueIri: IRI <-
        checkOrCreateEntityIri(newValueVersionIri, stringFormatter.makeRandomValueIri(resourceInfo.resourceIri))
    ...
    def makeTaskFuture: Future[SuccessResponseV2] = {
      for {
        ...
        cardinalityInfo: Cardinality.KnoraCardinalityInfo = classInfo.allCardinalities.getOrElse(
                                                              submittedInternalPropertyIri,
                                                              throw InconsistentRepositoryDataException(
                                                                s"Resource <${deleteValueRequest.resourceIri}> belongs to class <${resourceInfo.resourceClassIri.toOntologySchema(ApiV2Complex)}>, which has no cardinality for property <${deleteValueRequest.propertyIri}>"
                                                              )
                                                            )
