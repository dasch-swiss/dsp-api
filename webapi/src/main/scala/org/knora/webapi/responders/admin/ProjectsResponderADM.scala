...
class ProjectsResponderADM(responderData: ResponderData) extends Responder(responderData) with InstrumentationSupport {
    ...
    tracedFuture("admin-get-project") {
      ...
      for {
        maybeProjectADM <- if (skipCache) {
                             ...
                             getProjectFromCacheOrTriplestore(
                               identifier = identifier,
                               featureFactoryConfig = featureFactoryConfig
                             )
                           }
  ...
  private def projectAdminMembersGetRequestADM(
    ...
    for {
      ...
      _ = if (project.isEmpty) {
            ...
            if (
              !requestingUser.permissions.isSystemAdmin && !requestingUser.permissions.isProjectAdmin(project.get.id)
            ) {
              throw ForbiddenException("SystemAdmin or ProjectAdmin permissions are required.")
            }
          }
  ...
  private def projectDataGetRequestADM(
    ...
    for {
      ...
      project: ProjectADM =
        maybeProject.getOrElse(throw NotFoundException(s"Project '${projectIdentifier.value}' not found."))
  ...
  private def projectRestrictedViewSettingsGetADM(
    ...
    for {
      ...
      restrictedViewSettings = if (projectResponse.statements.nonEmpty) {
                                 ...
                                 val (_, propsMap): (SubjectV2, Map[SmartIri, Seq[LiteralV2]]) =
                                   projectResponse.statements.head
  ...
  private def projectCreateRequestADM(
    ...
    def projectCreateTask(
      ...
      for {
        ...
        newProjectIRI: IRI <-
          checkOrCreateEntityIri(customProjectIri, stringFormatter.makeRandomProjectIri(createProjectRequest.shortcode))
  ...
  private def getProjectFromTriplestore(
    ...
    for {
      ...
      maybeProjectADM: Option[ProjectADM] = if (projectResponse.statements.nonEmpty) {
                                              log.debug(
                                                "getProjectFromTriplestore - triplestore hit for: {}",
                                                identifier
                                              )
                                              val projectOntologies =
                                                ontologies.getOrElse(projectIris.head, Seq.empty[IRI])
                                              Some(
                                                statements2ProjectADM(
                                                  statements = projectResponse.statements.head,
                                                  ontologies = projectOntologies
                                                )
                                              )
                                            } else {
                                              log.debug(
                                                "getProjectFromTriplestore - no triplestore hit for: {}",
                                                identifier
                                              )
                                              None
                                            }
