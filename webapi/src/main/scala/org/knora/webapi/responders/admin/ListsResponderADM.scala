...
class ListsResponderADM(responderData: ResponderData) extends Responder(responderData) {
  ...
  protected def shiftNodes(
    ...
    for {
      ...
      nodesTobeUpdated: Seq[ListChildNodeADM] <-
        Future(nodes.filter(node => node.position >= startPos && node.position <= endPos))
      ...
      maybeList: Option[ListADM] <- if (exists) {
                                      for {
                                        ...
                                        rootNodeInfo = maybeRootNodeInfo match {
                                                         case Some(info: ListRootNodeInfoADM) =>
                                                           info.asInstanceOf[ListRootNodeInfoADM]
      ...
      maybeListNode: Option[ListNodeADM] <- if (listInfoResponse.statements.nonEmpty) {
                                              for {
                                                ...
                                                node: ListNodeADM = statements.head match {
                                                                      case (
                                                                            nodeIri: SubjectV2,
                                                                            propsMap: Map[SmartIri, Seq[LiteralV2]]
                                                                          ) =>
                                                                        val labels: Seq[StringLiteralV2] = propsMap
                                                                          .getOrElse(
                                                                            OntologyConstants.Rdfs.Label.toSmartIri,
                                                                        ...
                                                                        val isRootNode: Boolean = propsMap.get(
                                                                          OntologyConstants.KnoraBase.IsRootNode.toSmartIri
                                                                        ) match {
                                                                          case Some(values: Seq[LiteralV2]) =>
                                                                            values.headOption match {
                                                                              case Some(value: BooleanLiteralV2) =>
                                                                                value.value
                                                                              case Some(other) =>
                                                                                throw InconsistentRepositoryDataException(
                                                                                  s"Expected isRootNode as an BooleanLiteralV2 for list node $nodeIri, but got $other"
                                                                        ...
                                                                        val positionOption: Option[Int] = propsMap
                                                                          .get(
                                                                            OntologyConstants.KnoraBase.ListNodePosition.toSmartIri
                                                                          )
                                                                          .map(_.head.asInstanceOf[IntLiteralV2].value)
                                                                        ...
                                                                        if (isRootNode) {
                                                                          ListRootNodeADM(
                                                                            ...
                                                                            name = propsMap
                                                                              .get(
                                                                                OntologyConstants.KnoraBase.ListNodeName.toSmartIri
                                                                              )
                                                                              .map(
                                                                                _.head
                                                                            ...
                                                                            labels = StringLiteralSequenceV2(
                                                                              labels.toVector.sortBy(_.language)
                                                                            ),
                                                                            comments = StringLiteralSequenceV2(
                                                                              comments.toVector.sortBy(_.language)
                                                                            ),
                                                                            children = children
                                                                          )
                                                                        } else {
  ...
  private def getChildren(
    ...
    for {
      ...
      children: Seq[ListChildNodeADM] =
        startNodePropsMap.get(OntologyConstants.KnoraBase.HasSubListNode.toSmartIri) match {
          case Some(iris: Seq[LiteralV2]) =>
            iris.map { iri =>
              createChildNode(iri.toString, statements)
            }
  ...
  private def createNode(
    ...
    for {
      ...
      project: ProjectADM = maybeProject match {
                              case Some(project: ProjectADM) => project
                              case None =>
                                throw BadRequestException(s"Project '${createNodeRequest.projectIri}' not found.")
                            }
  ...
  private def getUpdateNodeInfoSparqlStatement(
    ...
    for {
      ...
      nodeNameUnique: Boolean <-
        listNodeNameIsProjectUnique(changeNodeInfoRequest.projectIri, changeNodeInfoRequest.name)
      _ = if (!nodeNameUnique) {
      ...
      node =
        maybeNode.getOrElse(throw BadRequestException(s"List item with '${changeNodeInfoRequest.listIri}' not found."))
