...
class OntologyResponderV1(responderData: ResponderData) extends Responder(responderData) {
  ...
  def getNamedGraphEntityInfoV1ForNamedGraph(namedGraphIri: IRI, userProfile: UserADM): Future[NamedGraphEntityInfoV1] =
    for {
      response: OntologyKnoraEntitiesIriInfoV2 <-
        (responderManager ? OntologyKnoraEntityIrisGetRequestV2(namedGraphIri.toSmartIri, userProfile))
  ...
  private def getStandoffStandoffClassesWithDataTypeV1(
    ...
    for {
      response: StandoffClassesWithDataTypeGetResponseV2 <-
        (responderManager ? StandoffClassesWithDataTypeGetRequestV2(userProfile))
  ...
  private def getAllStandoffPropertyEntities(userProfile: UserADM): Future[StandoffAllPropertiesGetResponseV1] =
    for {
      response: StandoffAllPropertyEntitiesGetResponseV2 <-
        (responderManager ? StandoffAllPropertyEntitiesGetRequestV2(userProfile))
  ...
  private def getResourceTypeResponseV1(
    ...
    for {
      ...
      resourceClassInfoResponse: EntityInfoGetResponseV1 <-
        getEntityInfoResponseV1(resourceClassIris = Set(resourceTypeIri), userProfile = userProfile)
  ...
  private def getResourceTypesForNamedGraph(
    ...
    def getResourceTypes(namedGraphIri: IRI): Future[Seq[ResourceTypeV1]] =
      for {
        ...
        namedGraphEntityInfo: NamedGraphEntityInfoV1 <-
          getNamedGraphEntityInfoV1ForNamedGraph(namedGraphIri, userProfile)
  ...
  private def getPropertyTypesForNamedGraph(
    ...
    def getPropertiesForNamedGraph(
      ...
      for {
        ...
        entities: EntityInfoGetResponseV1 <-
          getEntityInfoResponseV1(propertyIris = propertyIris, userProfile = userProfile)
