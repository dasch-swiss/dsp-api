...
import org.knora.webapi.messages.util.{ResponderData, ValueUtilV1}
...
class OntologyResponderV1(responderData: ResponderData) extends Responder(responderData) {
  ...
  /**
    ...
    */
  private def getEntityInfoResponseV1(resourceClassIris: Set[IRI] = Set.empty[IRI],
                                      propertyIris: Set[IRI] = Set.empty[IRI],
                                      userProfile: UserADM): Future[EntityInfoGetResponseV1] = {
    for {
      response: EntityInfoGetResponseV2 <- (responderManager ? EntityInfoGetRequestV2(
        ...
        userProfile)).mapTo[EntityInfoGetResponseV2]
    } yield
  ...
  /**
    ...
    * @return an [[EntityInfoGetResponseV1]].
    */
  private def getStandoffEntityInfoResponseV1(standoffClassIris: Set[IRI] = Set.empty[IRI],
                                              standoffPropertyIris: Set[IRI] = Set.empty[IRI],
                                              userProfile: UserADM): Future[StandoffEntityInfoGetResponseV1] = {
    for {
  ...
  private def getStandoffStandoffClassesWithDataTypeV1(
      userProfile: UserADM): Future[StandoffClassesWithDataTypeGetResponseV1] = {
    for {
  ...
  private def getAllStandoffPropertyEntities(userProfile: UserADM): Future[StandoffAllPropertiesGetResponseV1] = {
    for {
      response: StandoffAllPropertyEntitiesGetResponseV2 <- (responderManager ? StandoffAllPropertyEntitiesGetRequestV2(
        userProfile)).mapTo[StandoffAllPropertyEntitiesGetResponseV2]
    } yield
  ...
  /**
    ...
    */
  private def getResourceTypeResponseV1(resourceTypeIri: String,
                                        userProfile: UserADM): Future[ResourceTypeResponseV1] = {

    for {
      // Get all information about the resource type, including its property cardinalities.
      resourceClassInfoResponse: EntityInfoGetResponseV1 <- getEntityInfoResponseV1(resourceClassIris =
                                                                                      Set(resourceTypeIri),
      ...
      propertyDefinitions: Vector[PropertyDefinitionV1] = resourceClassInfo.knoraResourceCardinalities
        .filterNot {
      ...
      resourceTypeResponse = ResourceTypeResponseV1(
        restype_info = ResTypeInfoV1(
          ...
          label = resourceClassInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Label,
                                                       preferredLangs =
                                                         Some(userProfile.lang, settings.fallbackLanguage)),
          description = resourceClassInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Comment,
                                                             preferredLangs =
                                                               Some(userProfile.lang, settings.fallbackLanguage)),
  ...
  private def checkSubClass(checkSubClassRequest: CheckSubClassRequestV1): Future[CheckSubClassResponseV1] = {
    for {
      response: CheckSubClassResponseV2 <- (responderManager ? CheckSubClassRequestV2(
        ...
        checkSubClassRequest.userProfile)).mapTo[CheckSubClassResponseV2]
  ...
  private def getSubClasses(getSubClassesRequest: SubClassesGetRequestV1): Future[SubClassesGetResponseV1] = {
    for {
      response: SubClassesGetResponseV2 <- (responderManager ? SubClassesGetRequestV2(
        ...
        getSubClassesRequest.userADM)).mapTo[SubClassesGetResponseV2]
  ...
  private def getNamedGraphs(projectIris: Set[IRI] = Set.empty[IRI],
    ...
    for {
      ...
      readOntologyMetadataV2 <- (responderManager ? OntologyMetadataGetByProjectRequestV2(
        ...
        requestingUser = userProfile)).mapTo[ReadOntologyMetadataV2]
      ...
      namedGraphs: Seq[NamedGraphV1] = readOntologyMetadataV2.ontologies.toVector
        ...
        .map { ontologyMetadata =>
          ...
          NamedGraphV1(
            ...
            longname = project.longname.getOrElse(
              throw InconsistentRepositoryDataException(s"Project ${project.id} has no longname")),
  ...
  /**
    ...
    */
  def getNamedGraphEntityInfoV1ForNamedGraph(namedGraphIri: IRI,
                                             userProfile: UserADM): Future[NamedGraphEntityInfoV1] = {
    for {
      response: OntologyKnoraEntitiesIriInfoV2 <- (responderManager ? OntologyKnoraEntityIrisGetRequestV2(
        namedGraphIri.toSmartIri,
        userProfile)).mapTo[OntologyKnoraEntitiesIriInfoV2]
  ...
  private def getResourceTypesForNamedGraph(namedGraphIriOption: Option[IRI],
    ...
    def getResourceTypes(namedGraphIri: IRI): Future[Seq[ResourceTypeV1]] = {
      for {
        ...
        namedGraphEntityInfo: NamedGraphEntityInfoV1 <- getNamedGraphEntityInfoV1ForNamedGraph(namedGraphIri,
                                                                                               userProfile)

        // get resinfo for each resource class in namedGraphEntityInfo
        resInfosForNamedGraphFuture: Set[Future[(String, ResourceTypeResponseV1)]] = namedGraphEntityInfo.resourceClasses
          .map { resClassIri =>
            for {
          ...
          case (resClassIri, resInfo) =>
            val properties = resInfo.restype_info.properties.map { prop =>
              PropertyTypeV1(
                id = prop.id,
                label =
                  prop.label.getOrElse(throw InconsistentRepositoryDataException(s"No label given for ${prop.id}"))
              )
            }.toVector
  ...
  private def getPropertyTypesForNamedGraph(namedGraphIriOption: Option[IRI],
    ...
    def getPropertiesForNamedGraph(namedGraphIri: IRI,
                                   userProfile: UserADM): Future[Seq[PropertyDefinitionInNamedGraphV1]] = {
      for {
        ...
        entities: EntityInfoGetResponseV1 <- getEntityInfoResponseV1(propertyIris = propertyIris,
                                                                     userProfile = userProfile)
  ...
  /**
    ...
    */
  private def getPropertyTypesForResourceType(resourceClassIri: IRI,
                                              userProfile: UserADM): Future[PropertyTypesForResourceTypeResponseV1] = {
    for {
