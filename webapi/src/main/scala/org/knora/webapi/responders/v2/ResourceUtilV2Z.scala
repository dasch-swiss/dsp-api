...
trait ResourceUtilV2Z {
   ...
   *
   * @param task   the [[Task]] that should have updated the triplestore.
...
final case class ResourceUtilV2ZLive(
  messageRelay: MessageRelay,
  triplestoreService: TriplestoreService,
  implicit val stringFormatter: StringFormatter
) extends ResourceUtilV2Z {
   ...
   * If not, it asks Sipi to delete the temporary file.
   *
   * @param updateTask     the future that should have updated the triplestore.
   * @param valueContent   the value that should have been created or updated.
   * @param requestingUser the user making the request.
   */
      ...
      case fileValueContent: FileValueContentV2 =>
        ...
        updateTask.foldZIO(
          (_: Throwable) => {
            ...
            val sipiResponseFuture: Task[SuccessResponseV2] = messageRelay.ask[SuccessResponseV2](sipiRequest)

            // Did Sipi successfully delete the temporary file?
            sipiResponseFuture.foldZIO(
              (failure: Throwable) => {
                // No. Log Sipi's error, and return the future we were given.
                ZIO.logErrorCause("Sipi error", Cause.fail(failure)) *> updateTask
              },
              (_: SuccessResponseV2) => {
          ...
          (updateInProject: UpdateResultInProject) => {
...
object ResourceUtilV2ZLive {
  val layer: URLayer[MessageRelay with TriplestoreService with StringFormatter, ResourceUtilV2Z] =
    ZLayer.fromFunction(ResourceUtilV2ZLive.apply _)
}
