...
class ResourcesResponderV1(responderData: ResponderData) extends Responder(responderData) {
  ...
  def createResourceAndCheck(
    ...
    val updateFuture = for {
      ...
      defaultPropertyAccessPermissionsIterable: Iterable[(IRI, String)] <-
        Future.sequence(defaultPropertyAccessPermissionsFutures)
    ...
    def traverseGraph(
      ...
      for {
        ...
        recursiveResults: GraphQueryResults <- if (rows.isEmpty) {
                                                 ...
                                                 val visibleNodeIris =
                                                   otherNodes.map(_.nodeIri).toSet + startNode.nodeIri
                                                 ...
                                                 val edges: Set[QueryResultEdge] = rows.map { row =>
                                                   ...
                                                   QueryResultEdge(
                                                     ...
                                                     sourceNodeProject =
                                                       if (outbound) startNode.nodeProject else rowMap("nodeProject"),
                                                   ...
                                                   // nodes.
                                                   val hasPermission = visibleNodeIris.contains(
                                                     edge.sourceNodeIri
                                                   ) && visibleNodeIris.contains(edge.targetNodeIri) &&
                                                     PermissionUtilADM
                                                       .getUserPermissionV1(
                                                 ...
                                                 val visibleNodeIrisFromEdges =
                                                   edges.map(_.sourceNodeIri) ++ edges.map(_.targetNodeIri)
                                                 val filteredOtherNodes = otherNodes.filter(node =>
                                                   visibleNodeIrisFromEdges.contains(node.nodeIri)
                                                 )

                                                 // Make a GraphQueryResults containing the resulting nodes and edges, including the start
                                                 // node.
                                                 val results = GraphQueryResults(
                                                   nodes = filteredOtherNodes.toSet + startNode,
                                                   edges = edges
                                                 )
                                                 ...
                                                 if (depth == 1) {
                                                   ...
                                                   val lowerResultFutures: Seq[Future[GraphQueryResults]] =
                                                   ...
                                                   val lowerResultsFuture: Future[Seq[GraphQueryResults]] =
                                                     Future.sequence(lowerResultFutures)
  ...
  private def getFullResponseV1(
    ...
    for {
      ...
      maybeResourceTypeIconSrc =
        resourceTypeEntityInfo.getPredicateObject(OntologyConstants.KnoraBase.ResourceIcon) match {
      ...
      emptyPropsInfoResponse: EntityInfoGetResponseV1 <-
        (responderManager ? EntityInfoGetRequestV1(propertyIris = emptyPropsIris, userProfile = userProfile))
          .mapTo[EntityInfoGetResponseV1]
      ...
      properties: Seq[PropertyV1] = if (resInfo.locations.nonEmpty) {
                                      PropertyV1(
                                        ...
                                        guielement = Some(
                                          SalsahGuiConversions.iri2SalsahGuiElement(
                                            OntologyConstants.SalsahGui.Fileupload
                                          )
                                        ),
    ...
    for {
      ...
      resourceContexts: Seq[ResourceContextItemV1] <- if (containingResInfoV1Option.isEmpty) {
                                                        for {
                                                          ...
                                                          contextQueryResponse: SparqlSelectResult <-
                                                            (storeManager ? SparqlSelectRequest(contextSparqlQuery))
                                                              .mapTo[SparqlSelectResult]
                                                          rows: Seq[VariableResultsRow] =
                                                            contextQueryResponse.results.bindings

                                                          // The results consist of one row per source object.
                                                          sourceObjects: Seq[SourceObject] =
                                                            rows.map { row: VariableResultsRow =>
                                                              val sourceObject: IRI = row.rowMap("sourceObject")
                                                              val projectShortcode: String =
                                                                sourceObject.toSmartIri.getProjectCode
                                                                  .getOrElse(
                                                                    throw InconsistentRepositoryDataException(
                                                          ...
                                                          sourceObjectsWithPermissions =
                                                            sourceObjects.filter(sourceObj =>
                                                              sourceObj.permissionCode.nonEmpty
                                                            )

                                                          contextItems: Seq[ResourceContextItemV1] =
                                                            sourceObjectsWithPermissions.map {
      ...
      resinfoV1WithRegionsOption: Option[ResourceInfoV1] <- if (resinfo) {
                                                              ...
                                                              for {
                                                                ...
                                                                regionQueryResponse: SparqlSelectResult <-
                                                                ...
                                                                regionPropertiesSequencedFutures: Seq[
                                                                  Future[PropsGetForRegionV1]
                                                                      ...
                                                                      val permissionCodeForRegion =
                                                                        PermissionUtilADM.getUserPermissionV1(
                                                                          ...
                                                                          entityPermissionLiteral =
                                                                            regionRow.rowMap("regionObjectPermissions"),
                                                                          userProfile = userProfileV1
                                                                        )
                                                                ...
                                                                regionProperties: Seq[PropsGetForRegionV1] <-
                                                                  Future.sequence(regionPropertiesSequencedFutures)

                                                                resinfoWithRegions: Option[ResourceInfoV1] =
                                                                  if (regionProperties.nonEmpty) {
                                                                    // regions are given, append them to resinfo
                                                                    Some(
                                                                      resInfoV1.copy(regions = Some(regionProperties))
                                                                    )
                                                                  } else {
  ...
  private def getResourceSearchResponseV1(
    ...
    for {
            ...
            searchResultRow: ResourceSearchResultRowV1 = if (numberOfProps > 1) {
                                                           ...
                                                           maybeValues match {
                                                             case Some(valuesReturned) =>
                                                               val valueStrings = valuesReturned.split(
                                                               ...
                                                               val values: Seq[String] =
                                                                 (valueStrings, guiOrders, valueOrders).zipped.toVector
                                                             ...
                                                             case None =>
                                                               log.debug(
                                                                 "more values were asked (numberOfProps > 1), but there were none to be found"
                                                               )
                                                               ResourceSearchResultRowV1(
                                                                 id = row.rowMap("resourceIri"),
      ...
      filteredResources =
        resources.filter(_.rights.nonEmpty) // user must have permissions to see resource (must not be None)
  ...
  private def createMultipleNewResources(
    ...
    val updateFuture: Future[MultipleResourceCreateResponseV1] = for {
      ...
      readOntologyMetadataV2: ReadOntologyMetadataV2 <-
        (responderManager ? OntologyMetadataGetByIriRequestV2(resourceClassOntologyIris, requestingUser))
      ...
      defaultResourceClassAccessPermissionsSeq: Vector[(IRI, String)] <-
        Future.sequence(defaultResourceClassAccessPermissionsFutures)
      ...
      defaultPropertyAccessPermissionsMapContents: Map[IRI, Map[IRI, String]] <-
        ActorUtil.sequenceFuturesInMap(defaultPropertyAccessPermissionsFutures)
        ...
        resourcesToCreate.map { resourceCreateRequest: OneOfMultipleResourceCreateRequestV1 =>
          ...
          for {
            ...
            defaultObjectAccessPermissions: String <-
              FastFuture(Try(defaultResourceClassAccessPermissionsMap(resourceCreateRequest.resourceTypeIri)))
  ...
  private def createNewResource(
    ...
    for {
      ...
      readOntologyMetadataV2: ReadOntologyMetadataV2 <-
        (responderManager ? OntologyMetadataGetByIriRequestV2(Set(resourceClassOntologyIri), userProfile))
          .mapTo[ReadOntologyMetadataV2]
    ...
    if (resInfoResponseRows.isEmpty) {
      ...
      for {
        ...
        (restype_label, restype_description, restype_iconsrc) <- if (queryOntology) {
                                                                   val resTypeIri = groupedByPredicate(
  ...
  private def queryResults2PropertyV1s(
    ...
    for {
      valuePropertiesWithDataWithOption: Iterable[Option[PropertyV1]] <-
        Future.sequence(valuePropertiesWithDataWithFuture)
      ...
      linkPropertiesWithDataWithOption: Iterable[Option[PropertyV1]] <-
        Future.sequence(linkPropertiesWithDataWithFuture)
