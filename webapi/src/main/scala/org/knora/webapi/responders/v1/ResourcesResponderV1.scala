...
class ResourcesResponderV1(responderData: ResponderData) extends Responder(responderData) {
    ...
    private def getGraphDataResponseV1(graphDataGetRequest: GraphDataGetRequestV1): Future[GraphDataGetResponseV1] = {
        ...
        def traverseGraph(startNode: QueryResultNode, outbound: Boolean, depth: Int, traversedEdges: Set[QueryResultEdge] = Set.empty[QueryResultEdge]): Future[GraphQueryResults] = {
            ...
            for {
                ...
                sparql <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getGraphData(
        ...
        for {
            ...
            _ = if (PermissionUtilADM.getUserPermissionV1(
                ...
                throw ForbiddenException(s"User ${graphDataGetRequest.userADM.id} does not have permission to view resource ${graphDataGetRequest.resourceIri}")
            }
            ...
            entityInfoRequest = EntityInfoGetRequestV1(resourceClassIris = resourceClassIris, propertyIris = propertyIris, userProfile = graphDataGetRequest.userADM)
    ...
    private def getFullResponseV1(resourceIri: IRI, userProfile: UserADM, getIncoming: Boolean = true): Future[ResourceFullResponseV1] = {
        ...
        val maybeIncomingRefsFuture: Future[Option[SparqlSelectResponse]] = if (getIncoming) {
            for {
                incomingRefsSparql <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getIncomingReferences(
                    triplestore = settings.triplestoreType,
                    resourceIri = resourceIri
                ).toString())
        ...
        for {
            ...
            resourceClassLabel = resourceTypeEntityInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Label, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage))

            resInfo: ResourceInfoV1 = resInfoWithoutQueryingOntology.copy(
                restype_label = resourceClassLabel,
                restype_description = resourceTypeEntityInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Comment, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage)),
                ...
                incoming =>
                    ...
                    incoming.copy(
                        resinfo = incoming.resinfo.copy(
                            restype_label = incomingResourceTypeEntityInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Label, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage)),
                            restype_description = incomingResourceTypeEntityInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Comment, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage)),
    ...
    private def getContextResponseV1(resourceIri: IRI, userProfile: UserADM, resinfo: Boolean): Future[ResourceContextResponseV1] = {
        ...
        for {
            ...
            isPartOfSparqlQuery = org.knora.webapi.messages.queries.sparql.v1.txt.isPartOf(
                triplestore = settings.triplestoreType,
                resourceIri = resourceIri
            ).toString()
            ...
            resourceContexts: Seq[ResourceContextItemV1] <- if (containingResInfoV1Option.isEmpty) {
                for {
                    ...
                    contextSparqlQuery <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getContext(
                        triplestore = settings.triplestoreType,
                        resourceIri = resourceIri
                    ).toString())
            ...
            resinfoV1WithRegionsOption: Option[ResourceInfoV1] <- if (resinfo) {
                ...
                for {
                    ...
                    //
                    regionSparqlQuery <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getRegions(
                        triplestore = settings.triplestoreType,
                        resourceIri = resourceIri
                    ).toString())
    ...
    private def getResourceSearchResponseV1(searchString: String, resourceTypeIri: Option[IRI], numberOfProps: Int, limitOfResults: Int, userProfile: UserADM): Future[ResourceSearchResponseV1] = {
        ...
        for {
            ...
            searchResourcesSparql <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getResourceSearchResult(
                triplestore = settings.triplestoreType,
    ...
    def generateSparqlForNewResources(resourcesToCreate: Seq[SparqlTemplateResourceToCreate], projectIri: IRI, namedGraph: IRI, creatorIri: IRI): String = {
        ...
        org.knora.webapi.messages.queries.sparql.v1.txt.createNewResources(
    ...
    def verifyResourceCreated(resourceIri: IRI,
        ...
        for {
            createdResourcesSparql <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getCreatedResource(
                triplestore = settings.triplestoreType,
                resourceIri = resourceIri
            ).toString())
    ...
    private def deleteResourceV1(resourceDeleteRequest: ResourceDeleteRequestV1): Future[ResourceDeleteResponseV1] = {
        ...
        def makeTaskFuture(userIri: IRI): Future[ResourceDeleteResponseV1] = {
            for {
                ...
                sparqlUpdate = org.knora.webapi.messages.queries.sparql.v1.txt.deleteResource(
                ...
                sparqlQuery = org.knora.webapi.messages.queries.sparql.v1.txt.checkResourceDeletion(
                    triplestore = settings.triplestoreType,
                    resourceIri = resourceDeleteRequest.resourceIri
                ).toString()
    ...
    private def changeResourceLabelV1(resourceIri: IRI, label: String, apiRequestID: UUID, userProfile: UserADM): Future[ChangeResourceLabelResponseV1] = {
        ...
        def makeTaskFuture(userIri: IRI): Future[ChangeResourceLabelResponseV1] = {
            ...
            for {
                ...
                sparqlUpdate = org.knora.webapi.messages.queries.sparql.v1.txt.changeResourceLabel(
                    dataNamedGraph = namedGraph,
                    triplestore = settings.triplestoreType,
                    resourceIri = resourceIri,
                    label = label,
                ...
                // Check whether the update succeeded.
                sparqlQuery = org.knora.webapi.messages.queries.sparql.v1.txt.checkResourceLabelChange(
                    triplestore = settings.triplestoreType,
                    resourceIri = resourceIri,
                    label = label
                ).toString()
    ...
    private def getResourceInfoV1(resourceIri: IRI, userProfile: UserADM, queryOntology: Boolean): Future[(Option[Int], ResourceInfoV1)] = {
        for {
            sparqlQuery <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getResourceInfo(
                triplestore = settings.triplestoreType,
                resourceIri = resourceIri
            ).toString())
    ...
    private def getPropertiesV1(resourceIri: IRI, userProfile: UserADM): Future[PropertiesGetResponseV1] = {
        ...
        for {
            ...
            resclassSparqlQuery <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getResourceClass(
                triplestore = settings.triplestoreType,
                resourceIri = resourceIri
            ).toString())
            ...
            for {
                ...
                valuePropsForFileValues: Seq[(IRI, ValueProps)] = fileValueGroupedRows.map {
                    ...
                    case (fileValueIri, fileValueProps) =>
                        ...
                        PermissionUtilADM.impliesPermissionCodeV1(userHasPermissionCode = permissionCode, userNeedsPermission = OntologyConstants.KnoraBase.RestrictedViewPermission)
                }
    ...
    private def getGroupedProperties(resourceIri: IRI): Future[GroupedPropertiesByType] = {
        ...
        for {
            sparqlQuery <- Future(org.knora.webapi.messages.queries.sparql.v1.txt.getResourcePropertiesAndValues(
                triplestore = settings.triplestoreType,
                resourceIri = resourceIri
            ).toString())
    ...
    private def queryResults2PropertyV1s(containingResourceIri: IRI,
        ...
        def makePropertyV1(propertyIri: IRI, propertyCardinality: Option[KnoraCardinalityInfo], propertyEntityInfo: Option[PropertyInfoV1], valueObjects: Seq[ValueObjectV1]): PropertyV1 = {
            PropertyV1(
                ...
                label = propertyEntityInfo.flatMap(_.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Label, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage))),
            ...
            case (propertyIri: IRI, valueObject: ValueObjects) =>
                ...
                // is MustHaveOne or MayHaveOne, don't return any information about the property.
                    propsAndCardinalities.get(propertyIri) match {
                        case Some(cardinalityInfo) if (cardinalityInfo.cardinality == Cardinality.MustHaveOne || cardinalityInfo.cardinality == Cardinality.MayHaveOne) && valueObjectsV1Sorted.nonEmpty && valueObjectListFiltered.isEmpty => None
            ...
            linkPropertiesWithDataWithFuture: Iterable[Future[Option[PropertyV1]]] = groupedPropertiesByType.groupedLinkProperties.groupedProperties.map {
                case (propertyIri: IRI, targetResource: ValueObjects) =>
                        ...
                        case (targetResourceIri: IRI, valueProps: ValueProps) =>
                            ...
                            val (maybeResourceClassLabel: Option[String], maybeResourceClassIcon: Option[String]) = resourceEntityInfoMap.get(referencedResType) match {
                                case Some(referencedResTypeEntityInfo) =>
                                    val labelOption: Option[String] = referencedResTypeEntityInfo.getPredicateObject(predicateIri = OntologyConstants.Rdfs.Label, preferredLangs = Some(userProfile.lang, settings.fallbackLanguage))
                    ...
                    for {
                        ...
                        case Some(cardinalityInfo) if (cardinalityInfo.cardinality == Cardinality.MustHaveOne || cardinalityInfo.cardinality == Cardinality.MayHaveOne) && valueObjectsV1.nonEmpty && valueObjectListFiltered.isEmpty => None
                        case _ => Some(propertyV1)
                    }
            }
