...
class ResourcesResponderV1 extends Responder {
    ...
    private def getGraphDataResponseV1(graphDataGetRequest: GraphDataGetRequestV1): Future[GraphDataGetResponseV1] = {
        val userProfileV1 = graphDataGetRequest.userProfile.asUserProfileV1
        ...
        for {
            ...
            _ = if (PermissionUtilADM.getUserPermissionV1(
                ...
                throw ForbiddenException(s"User ${graphDataGetRequest.userProfile.id} does not have permission to view resource ${graphDataGetRequest.resourceIri}")
            }
            ...
            resultNodes: Vector[GraphNodeV1] = nodes.map {
                node =>
                    ...
                    val resourceClassLabel = entityInfoResponse.resourceClassInfoMap(node.nodeClass).getPredicateObject(
                        ...
                        preferredLangs = Some(graphDataGetRequest.userProfile.lang, settings.fallbackLanguage)
    ...
    private def getContextResponseV1(resourceIri: IRI, userProfile: UserADM, resinfo: Boolean): Future[ResourceContextResponseV1] = {
            ...
            row.rowMap.get("fileValue") match {
                case Some(fileValueIri) =>
                    val fileValuePermission = PermissionUtilADM.getUserPermissionV1(
                        ...
                        subjectCreator = row.rowMap("fileValueAttachedToUser"),
                        subjectProject = fileValueProject,
                        subjectPermissionLiteral = row.rowMap("fileValuePermissions"),
                        userProfile = userProfileV1
                    )
        ...
        def createSourceObjectFromResultRow(row: VariableResultsRow): SourceObject = {
            ...
            val sourceObjectPermissionCode = PermissionUtilADM.getUserPermissionV1(
                ...
                subjectCreator = sourceObjectOwner,
                    ...
                    regionPropertiesSequencedFutures: Seq[Future[PropsGetForRegionV1]] = regionRows.filter {
                        regionRow =>
                            val permissionCodeForRegion = PermissionUtilADM.getUserPermissionV1(
                                ...
                                subjectCreator = regionRow.rowMap("owner"),
                                subjectProject = regionRow.rowMap("project"),
                                subjectPermissionLiteral = regionRow.rowMap("regionObjectPermissions"),
                                userProfile = userProfileV1
                            )
    ...
    private def deleteResourceV1(resourceDeleteRequest: ResourceDeleteRequestV1): Future[ResourceDeleteResponseV1] = {
        ...
        for {
            ...
            userIri <- Future {
                if (resourceDeleteRequest.userProfile.isAnonymousUser) {
                    throw ForbiddenException("Anonymous users aren't allowed to mark resources as deleted")
                } else {
                    resourceDeleteRequest.userProfile.id
                }
            }
