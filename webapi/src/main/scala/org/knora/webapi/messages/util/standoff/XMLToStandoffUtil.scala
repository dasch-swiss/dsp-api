...
class XMLToStandoffUtil(xmlNamespaces: Map[String, IRI] = Map.empty[IRI, String],
  ...
  private val xmlNamespaces2Prefixes = new ErrorHandlingMap(
    xmlNamespaces.map(_.swap), { key: String =>
  ...
  def xml2TextWithStandoff(xmlStr: String,
    ...
    val xmlStrWithSeparator = if (tagsWithSeparator.nonEmpty) {
      ...
      val xPAthExpression
        : String = tagsWithSeparator.map(_.toXPath).mkString("|") // build a union of the collected elements' expressions
      ...
      case Some(children) if children.size == 1 =>
        standoffTags2XmlString(
          text = textWithStandoff.text.replace(
            ...
            StringFormatter.PARAGRAPH_SEPARATOR.toString), // replace information separator (which is an invalid XML character) with paragraph separator
    ...
    for (standoffDiff <- standoffDiffs) {
      standoffDiff match {
        ...
        case insert: StandoffDiffInsert =>
          stringBuilder
            .append("<ins>")
            .append(StringEscapeUtils.escapeXml11(
  ...
  /**
    ...
    */
  def findChangedStandoffTags(oldStandoff: Seq[StandoffTag],
                              newStandoff: Seq[StandoffTag]): (Set[StandoffTag], Set[StandoffTag]) = {
  ...
  private case class Xml2StandoffState(currentPos: Int = 0,
                                       ...
                                       clixStartMilestones: Map[String, ClixMilestoneTag] =
                                         Map.empty[String, ClixMilestoneTag])
