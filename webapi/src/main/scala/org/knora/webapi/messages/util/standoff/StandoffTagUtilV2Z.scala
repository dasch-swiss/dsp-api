...
trait StandoffTagUtilV2Z {
  ...
  def createStandoffTagsV2FromSelectResults(
    standoffAssertions: Map[IRI, Map[IRI, IRI]],
    requestingUser: UserADM
  ): Task[Vector[StandoffTagV2]]
}
final case class StandoffTagUtilV2ZLive(messageRelay: MessageRelay) extends StandoffTagUtilV2Z {
      ...
      val attributes: Seq[StandoffTagAttribute] = standoffTagV2.dataType match {
        ...
        case Some(StandoffDataTypeClasses.StandoffInternalReferenceTag) =>
          ...
          convertStandoffAttributeTags(
            ...
            value = internalLinkMarker.toString + internalRefTarget,
            xmlNamespace = None
          )
...
object StandoffTagUtilV2ZLive {
  val layer: ZLayer[MessageRelay, Nothing, StandoffTagUtilV2ZLive] = ZLayer.fromFunction(StandoffTagUtilV2ZLive(_))
}
