              ...
              case AllInference => // TODO: get rid of duplicate code
                ontologyCache.getCacheData.flatMap { ontoCache =>
                  ZIO.attempt {
                    ...
                    if (iriRef.iri.toString == OntologyConstants.Rdf.Type) {
                      ...
                      val knownSubClasses = ontoCache.classToSubclassLookup
                        .getOrElse(baseClassIri.iri, Set(baseClassIri.iri))
                        .toSeq
                      unionize(statementPattern, knownSubClasses)
                    } else {
                      ...
                      val knownSubProps = superProps
                        .getOrElse(iriRef.iri, Set(iriRef.iri))
                        .toSeq
                      unionize(statementPattern, knownSubProps)
                    }
                  }
                }
              case NoInference                                => ZIO.succeed(Seq(statementPattern))
              case LimitedInference(projectIri, ontologyIris) =>
                // TODO: handle case where ontologyIris is empty (with and without projectIri)
                ...
                ontologyCache.getCacheData.flatMap { ontoCache =>
                  ZIO.attempt {
                    ...
                    if (iriRef.iri.toString == OntologyConstants.Rdf.Type) {
                      ...
                      val relevantSubClasses = knownSubClasses.filter { subClass =>
                        ontoCache.classDefinedInOntology.get(subClass) match {
                          case Some(ontologyOfSubclass) =>
                            ...
                            ontologyIris.contains(ontologyOfSubclass)
