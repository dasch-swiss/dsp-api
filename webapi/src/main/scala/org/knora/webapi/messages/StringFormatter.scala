    ...
    def makeUnusedIriRec(attempts: Int): Future[IRI] = {
      ...
      for {
        ...
        result <- if (!iriExists) {
                    ...
                    log.warning(
                      "KnoraIdUtil.makeUnusedIri generated an IRI that already exists in the triplestore, retrying"
                    )
                    makeUnusedIriRec(attempts - 1)
                  } else {
                    throw UpdateNotPerformedException(
                      s"Could not make an unused new IRI after $MAX_IRI_ATTEMPTS attempts"
                    )
                  }
      } yield result
    }
