...
case class IntermediateTypeInspectionResult(
  ...
  /**
    ...
    */
  def addTypes(entity: TypeableEntity,
               entityTypes: Set[GravsearchEntityTypeInfo],
               inferredFromPropertyIri: Boolean = false): IntermediateTypeInspectionResult = {
    if (entityTypes.nonEmpty) {
      ...
      val newEntitiesInferredFromPropertyIris = if (inferredFromPropertyIri && entityTypes.nonEmpty) {
        val newTypesInferredFromPropertyIris = entitiesInferredFromPropertyIris.getOrElse(
          entity,
          Set.empty[GravsearchEntityTypeInfo]) ++ entityTypes
...
object IntermediateTypeInspectionResult {
  ...
  /**
    ...
    */
  def apply(entities: Set[TypeableEntity])(
