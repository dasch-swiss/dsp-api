...
object SparqlTransformer {
  ...
  class NoInferenceConstructToConstructTransformer(
    ...
    override def transformStatementInWhere(
      ...
      sparqlTransformerLive.transformStatementInWhereForNoInference(
        statementPattern = statementPattern,
        inference = LimitInference.make(None, limitInferenceToOntologies.map(_.toList).toList.flatten.map(_.toString))
      )
      ...
      case _ =>
        ...
        statementPattern.namedGraph match {
          ...
          case _ =>
            inference match {
              ...
              case AllInference =>
                statementPattern.pred match {
                  ...
                  case _                 => ZIO.succeed(Seq(statementPattern))
                }
            ...
            // XXX: move the following to the above question marks
