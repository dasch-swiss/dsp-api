 ...
 */
case class LoadOntologiesRequestV1(featureFactoryConfig: FeatureFactoryConfig, userADM: UserADM)
 ...
 */
case class EntityInfoGetRequestV1(
 ...
 */
case class EntityInfoGetResponseV1(
 ...
 * @param userProfile          the profile of the user making the request.
 */
case class StandoffEntityInfoGetRequestV1(
 ...
 */
case class StandoffEntityInfoGetResponseV1(
  standoffClassInfoMap: Map[IRI, ClassInfoV1],
 ...
 */
case class CheckSubClassRequestV1(subClassIri: IRI, superClassIri: IRI, userProfile: UserADM)
    extends OntologyResponderRequestV1
 ...
 * @param userADM              the profile of the user making the request.
 */
case class NamedGraphsGetRequestV1(
 ...
 * @param userADM              the profile of the user making the request.
 */
case class ResourceTypesForNamedGraphGetRequestV1(
  namedGraph: Option[IRI],
  featureFactoryConfig: FeatureFactoryConfig,
  userADM: UserADM
) extends OntologyResponderRequestV1
 ...
 * @param userProfile      the profile of the user making the request.
 */
case class PropertyTypesForResourceTypeGetRequestV1(resourceClassIri: IRI, userProfile: UserADM)
    extends OntologyResponderRequestV1
...
class PredicateInfoV1(predicateInfoV2: PredicateInfoV2) {
   ...
   */
  def objects: Set[String] =
    predicateInfoV2.objects.filter {
   ...
   */
  def objectsWithLang: Map[String, String] =
    predicateInfoV2.objects.collect { case StringLiteralV2(str, Some(lang)) =>
      lang -> str
    }.toMap
}
...
class ClassInfoV1(classInfoV2: ReadClassInfoV2) extends EntityInfoV1 {
  ...
  def allCardinalities: Map[IRI, KnoraCardinalityInfo] =
    classInfoV2.allCardinalities.map { case (smartIri, cardinality) =>
      smartIri.toString -> cardinality
    }
  ...
  def knoraResourceCardinalities: Map[IRI, KnoraCardinalityInfo] =
    classInfoV2.allResourcePropertyCardinalities.map { case (smartIri, cardinality) =>
      smartIri.toString -> cardinality
    }
...
object ConvertOntologyClassV2ToV1 {
  ...
  def classInfoMapV2ToV1(classInfoMap: Map[SmartIri, ReadClassInfoV2]): Map[IRI, ClassInfoV1] =
    classInfoMap.map { case (smartIri, classInfoV2) =>
      smartIri.toString -> new ClassInfoV1(classInfoV2)
    }
  ...
  def propertyInfoMapV2ToV1(propertyInfoMap: Map[SmartIri, ReadPropertyInfoV2]): Map[IRI, PropertyInfoV1] =
    propertyInfoMap.map { case (smartIri, propertyInfoV2) =>
      smartIri.toString -> new PropertyInfoV1(propertyInfoV2)
    }
 ...
 */
case class NamedGraphEntityInfoV1(namedGraphIri: IRI, resourceClasses: Set[IRI], propertyIris: Set[IRI])
 ...
 */
case class PropertyDefinitionV1(
  id: IRI,
  name: IRI,
  label: Option[String],
  description: Option[String],
...
) extends PropertyDefinitionBaseV1
 ...
 *                     that should be used for inputting values for this property.
 */
case class PropertyDefinitionInNamedGraphV1(
  id: IRI,
  name: IRI,
  label: Option[String],
  description: Option[String],
...
object ResourceTypeV1JsonProtocol extends SprayJsonSupport with DefaultJsonProtocol with NullOptions {
  ...
  implicit val propertyTypesForNamedGraphResponseV1Format: RootJsonFormat[PropertyTypesForNamedGraphResponseV1] =
    jsonFormat1(PropertyTypesForNamedGraphResponseV1)
  implicit val propertyTypesForResourceTypeResponseV1Format: RootJsonFormat[PropertyTypesForResourceTypeResponseV1] =
    jsonFormat1(PropertyTypesForResourceTypeResponseV1)
  implicit val subClassInfoV1Format: JsonFormat[SubClassInfoV1] = jsonFormat2(SubClassInfoV1)
  implicit val subClassesGetResponseV1Format: RootJsonFormat[SubClassesGetResponseV1] = jsonFormat1(
    SubClassesGetResponseV1
  )
}
