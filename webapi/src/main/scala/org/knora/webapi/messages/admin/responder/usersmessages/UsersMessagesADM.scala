...
case class UserADM(id: IRI,
    ...
    def asUserProfileV1: UserProfileV1 = {
        ...
        if (this.isAnonymousUser) {
            ...
            val projectesWithoutBuiltinProjects = projects.filter(_.id != OntologyConstants.KnoraBase.SystemProject).filter(_.id != OntologyConstants.KnoraBase.SharedOntologiesProject)
            val projectInfosV1 = projectesWithoutBuiltinProjects.map(_.asProjectInfoV1)
