...
object ValueContentV2 extends ValueContentReaderV2[ValueContentV2] {
  ...
  override def fromJsonLDObject(
    ...
    for {
      valueType: SmartIri <-
        Future(jsonLDObject.requireStringWithValidation(JsonLDKeywords.TYPE, stringFormatter.toSmartIriWithErr))
  ...
  def fromJsonLDObject(
    ...
    for {
      ...
      fileMetadataResponse: GetFileMetadataResponse <-
        (storeManager ? GetFileMetadataRequest(fileUrl = tempFileUrl, requestingUser = requestingUser))
