...
/**
 * An custom-formatted text value, as read from the triplestore.
 *
 * @param valueIri                      the IRI of the value.
 * @param attachedToUser                the user that created the value.
 * @param permissions                   the permissions that the value grants to user groups.
 ...
 *                                      deleted and the reason why it was deleted.
 */
case class ReadCustomFormattedTextValueV2(
  valueIri: IRI,
  attachedToUser: IRI,
  permissions: String,
  userPermission: EntityPermission,
  valueCreationDate: Instant,
  valueHasUUID: UUID,
  valueContent: CustomFormattedTextValueContentV2,
  valueHasMaxStandoffStartIndex: Option[Int],
  previousValueIri: Option[IRI],
  deletionInfo: Option[DeletionInfo]
) extends ReadValueV2
    with KnoraReadV2[ReadCustomFormattedTextValueV2] {
   ...
   * @param targetSchema the target schema.
   */
  override def toOntologySchema(targetSchema: ApiV2Schema): ReadCustomFormattedTextValueV2 =
    copy(valueContent = valueContent.toOntologySchema(targetSchema))
      ...
      for {
        ...
        valueContent <-
          valueType.toString match {
            ...
            case CustomFormattedTextValue =>
              CustomFormattedTextValueContentV2.fromJsonLdObject(
                jsonLdObject,
                requestingUser
              )
 ...
 */
case class FormattedTextValueContentV2( // TODO: can only have standard mapping, so no reason to pass it here
  ontologySchema: OntologySchema,
  valueHasString: String,
  valueHasLanguage: Option[String] = None,
...
object FormattedTextValueContentV2 {
  ...
  private def getTextValue(
    ...
    for {
      _ <- ZIO
             .fail(BadRequestException("FormattedTextValues must use StandardMapping"))
             .unless(mappingResponse.mappingIri == OntologyConstants.KnoraBase.StandardMapping)
 ...
 * @param mappingIri     the IRI of the [[MappingXMLtoStandoff]] used with the text value.
 * @param mapping        the [[MappingXMLtoStandoff]] used with the text value.
 * @param comment        a comment on this [[CustomFormattedTextValueContentV2]], if any.
 */
case class CustomFormattedTextValueContentV2(
  ontologySchema: OntologySchema,
  valueHasString: String,
  valueHasLanguage: Option[String] = None,
  standoff: Seq[StandoffTagV2] = Vector.empty,
  ...
  override def valueType: SmartIri = {
    implicit val stringFormatter: StringFormatter = StringFormatter.getGeneralInstance
    OntologyConstants.KnoraBase.CustomFormattedTextValue.toSmartIri.toOntologySchema(ontologySchema)
  }
  ...
  /**
   * The maximum start index in the standoff attached to this [[CustomFormattedTextValueContentV2]]. This is used
   * only when writing a text value to the triplestore.
   */
  lazy val computedMaxStandoffStartIndex: Option[Int] = if (standoff.nonEmpty) {
    Some(standoff.map(_.startIndex).max)
  ...
  override def toOntologySchema(targetSchema: OntologySchema): CustomFormattedTextValueContentV2 =
    copy(ontologySchema = targetSchema)
  ...
  override def wouldDuplicateOtherValue(that: ValueContentV2): Boolean =
    ...
    // containing the same text but different markup.
    that match {
      case thatTextValue: CustomFormattedTextValueContentV2 => valueHasString == thatTextValue.valueHasString
      case _                                                => throw AssertionException(s"Can't compare a <$valueType> to a <${that.valueType}>")
    }
  ...
  override def wouldDuplicateCurrentVersion(currentVersion: ValueContentV2): Boolean =
    ...
    // or the comment.
    currentVersion match {
      case thatTextValue: CustomFormattedTextValueContentV2 =>
        val valueHasStringIdentical: Boolean = valueHasString == thatTextValue.valueHasString
        val mappingIdentitcal                = mappingIri == thatTextValue.mappingIri
        // compare standoff nodes (sort them first by index) and the XML-to-standoff mapping IRI
        val standoffIdentical = StandoffTagUtilV2.makeComparableStandoffCollection(standoff) == StandoffTagUtilV2
...
/**
 * Constructs [[CustomFormattedTextValueContentV2]] objects based on JSON-LD input.
 */
object CustomFormattedTextValueContentV2 {
  private def getSparqlEncodedString(
    obj: JsonLDObject,
    key: String
  ): ZIO[StringFormatter, BadRequestException, Option[IRI]] =
  ...
  private def getTextValue(
    ...
    for {
      _ <- ZIO
             .fail(BadRequestException("CustomFormattedTextValues cannot use StandardMapping"))
             .when(mappingResponse.mappingIri == OntologyConstants.KnoraBase.StandardMapping)
      textWithStandoffTags <- ZIO.attempt(
                                StandoffTagUtilV2.convertXMLtoStandoffTagV2(
                                  textValueAsXml,
                                  mappingResponse,
  ...
  /**
   * Converts a JSON-LD object to a [[CustomFormattedTextValueContentV2]].
   *
   * @param jsonLdObject         the JSON-LD object.
   * @param requestingUser       the user making the request.
  ...
  def fromJsonLdObject(
    jsonLdObject: JsonLDObject,
    requestingUser: UserADM
  ): ZIO[StringFormatter with MessageRelay, Throwable, CustomFormattedTextValueContentV2] =
    ZIO.serviceWithZIO[StringFormatter] { stringFormatter =>
      for {
        maybeValueHasLanguage <- getSparqlEncodedString(jsonLdObject, TextValueHasLanguage)
        textValueAsXml <- jsonLdObject
