 ...
 * Tests [[JsonLDTool]].
 */
abstract class JsonLDToolSpec(featureToggle: FeatureToggle) extends CoreSpec {
    ...
    "The JSON-LD tool" should {
        "parse JSON-LD text, compact it with an empty context, convert the result to a JsonLDDocument, and convert that back to text" in {
            ...
            val compactedJsonLDDoc: JsonLDDocument = JsonLDTool.parseJsonLD(inputStr)
        ...
        "convert JSON-LD representing an ontology to an RDF4J Model" in {
            ...
            val jsonLDDocument: JsonLDDocument = JsonLDTool.parseJsonLD(inputJsonLD)
            ...
            val expectedModel: RdfModel = rdfFormatTool.parseToRdfModel(expectedTurtle, Turtle)
        ...
        "convert an RDF4J Model representing an ontology to JSON-LD" in {
            ...
            val inputModel: RdfModel = rdfFormatTool.parseToRdfModel(turtle, Turtle)

            // Convert the model to a JsonLDDocument.
            val outputJsonLD: JsonLDDocument = JsonLDTool.fromRdfModel(inputModel)
            ...
            // Parse it to an RDF4J Model.
            val jsonLDExpectedModel: RdfModel = rdfFormatTool.parseToRdfModel(expectedJsonLD, JsonLD)
        ...
        "convert JSON-LD representing a resource to an RDF4J Model" in {
            ...
            val outputModelAsJsonLDDocument: JsonLDDocument = JsonLDTool.fromRdfModel(outputModel)
        ...
        "convert an RDF4J Model representing a resource to JSON-LD" in {
            ...
            val expectedJsonLDDocument: JsonLDDocument = JsonLDTool.parseJsonLD(expectedJsonLD)
