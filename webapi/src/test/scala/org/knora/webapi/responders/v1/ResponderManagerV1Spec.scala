...
class ResponderManagerV1Spec extends CoreSpec("ResponderManagerTestSystem") with ImplicitSender with Authenticator {
    ...
    val actorUnderTest = TestActorRef(Props(new ResponderManagerV1 with TestProbeMaker), RESPONDER_MANAGER_V1_ACTOR_NAME)
    ...
    val mockListsRouter = actorUnderTest.underlyingActor.asInstanceOf[TestProbeMaker].probes.getOrElse(HIERARCHICAL_LISTS_ROUTER_V1_ACTOR_NAME, null)
