  ...
  "The Permissions Route ('admin/permissions')" when {
    ...
    "creating permissions" should {
      "create an administrative access permission" in {
        ...
        val request = Post(baseApiUrl + s"/admin/permissions/ap", HttpEntity(ContentTypes.`application/json`, createAdministrativePermissionRequest)) ~> addCredentials(BasicHttpCredentials(
          SharedTestDataADM.rootUser.email, SharedTestDataADM.testPass))
      ...
      "create a default object access permission" in {
        ...
        val request = Post(baseApiUrl + s"/admin/permissions/doap", HttpEntity(ContentTypes.`application/json`, createDefaultObjectAccessPermissionRequest)) ~> addCredentials(BasicHttpCredentials(
          SharedTestDataADM.rootUser.email, SharedTestDataADM.testPass))
        val response: HttpResponse = singleAwaitingRequest(request)
        assert(response.status === StatusCodes.OK)
      ...
      "create a default object access permission with a custom IRI" in {
        ...
        val request = Post(baseApiUrl + s"/admin/permissions/doap", HttpEntity(ContentTypes.`application/json`,
          createDefaultObjectAccessPermissionWithCustomIriRequest)) ~> addCredentials(BasicHttpCredentials(
          SharedTestDataADM.rootUser.email, SharedTestDataADM.testPass))
        val response: HttpResponse = singleAwaitingRequest(request)
        assert(response.status === StatusCodes.OK)
    ...
    "updating permissions" should {
      ...
      "change the set of hasPermissions of an administrative permission" in {
        ...
        val request = Put(baseApiUrl + s"/admin/permissions/" + encodedPermissionIri + "/hasPermissions", HttpEntity(ContentTypes.`application/json`, updateHasPermissions)) ~> addCredentials(BasicHttpCredentials(
          SharedTestDataADM.rootUser.email, SharedTestDataADM.testPass))
        val response: HttpResponse = singleAwaitingRequest(request)
        assert(response.status === StatusCodes.OK)
        val result = AkkaHttpUtils.httpResponseToJson(response).fields("administrative_permission").asJsObject.fields
        val permissions = result.getOrElse("hasPermissions", throw DeserializationException("The expected field 'hasPermissions' is missing.")).asInstanceOf[JsArray].elements
      ...
      "change the set of hasPermissions of a default object access permission" in {
        ...
        val permissions = result.getOrElse("hasPermissions", throw DeserializationException("The expected field 'hasPermissions' is missing.")).asInstanceOf[JsArray].elements
        permissions.size should be(1)
        assert(permissions.head.asJsObject.fields("additionalInformation").toString.contains("http://www.knora.org/ontology/knora-admin#ProjectMember"))
        clientTestDataCollector.addFile(
          TestDataFileContent(
            filePath = TestDataFilePath(
              directoryPath = clientTestDataPath,
