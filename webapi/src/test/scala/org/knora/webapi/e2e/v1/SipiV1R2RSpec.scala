...
import org.knora.webapi.messages.triplestoremessages.{RdfDataObject, ResetTriplestoreContent}
...
class SipiV1R2RSpec extends R2RSpec {
    ...
    private val responderManager = system.actorOf(Props(new TestResponderManager(Map(SIPI_ROUTER_ACTOR_NAME -> system.actorOf(Props(new MockSipiResponderV1))))), name = RESPONDER_MANAGER_ACTOR_NAME)
