...
class ResourcesRouteV2E2ESpec extends E2ESpec(ResourcesRouteV2E2ESpec.config) {
  ...
  "The resources v2 endpoint" should {
    "perform a resource request for the book 'Reise ins Heilige Land' using the complex schema in JSON-LD" in {
      val request = Get(
        s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/6NIOckxUlWnYChtXoH7A_g/2a6221216701", "UTF-8")}")
    ...
    "perform a resource preview request for the book 'Reise ins Heilige Land' using the complex schema" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resourcespreview/${URLEncoder.encode("http://rdfh.ch/6NIOckxUlWnYChtXoH7A_g/2a6221216701", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "perform a resource preview request for a Thing resource using the complex schema" in {
      val request = Get(
        s"$baseApiUrl/v2/resourcespreview/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-thing", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "perform a resource request for the book 'Reise ins Heilige Land' using the simple schema (specified by a URL parameter)" in {
      val request =
        Get(s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/6NIOckxUlWnYChtXoH7A_g/2a6221216701",
                                                           "UTF-8")}?${RouteUtilV2.SCHEMA_PARAM}=${RouteUtilV2.SIMPLE_SCHEMA_NAME}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "perform a resource request for the first page of the book '[Das] Narrenschiff (lat.)' using the complex schema" in {
      val request = Get(
        s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/6NIOckxUlWnYChtXoH7A_g/7bbb8e59b703", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
    ...
    "perform a full resource request for a resource with a BCE date property" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing_with_BCE_date", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD = readOrWriteTextFile(responseAsString,
    ...
    "perform a full resource request for a resource with a date property that represents a period going from BCE to CE" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing_with_BCE_date2", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD = readOrWriteTextFile(responseAsString,
    ...
    "perform a full resource request for a resource with a list value" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing_with_list_value", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD = readOrWriteTextFile(responseAsString,
    ...
    "perform a full resource request for a resource with a link (in the complex schema)" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/0C-0L1kORryKzJAJxxRyRQ", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD = readOrWriteTextFile(responseAsString,
    ...
    "perform a full resource request for a resource with a Text language (in the complex schema)" in {
      val request = Get(s"$baseApiUrl/v2/resources/${URLEncoder
        .encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-thing-with-text-valuesLanguage", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "perform a full resource request for a resource with values of different types" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/H6gBWUuJSuuO-CilHV8kQw", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "perform a full resource request for a Thing resource with a link to a ThingPicture resource" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-thing-with-picture", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD = readOrWriteTextFile(responseAsString,
    ...
    "perform a full resource request with a link to a resource that the user doesn't have permission to see" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/0JhgKcqoRIeRRG6ownArSw", "UTF-8")}")
    ...
    "perform a full resource request with a link to a resource that is marked as deleted" in {
      val request =
        Get(
          s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/l8f8FVEiSCeq9A1p8gBR-A", "UTF-8")}")
    ...
    "perform a full resource request for a past version of a resource, using a URL-encoded xsd:dateTimeStamp" in {
      val request = Get(
        s"$baseApiUrl/v2/resources/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing-with-history", "UTF-8")}?version=${URLEncoder
    ...
    "return the complete version history of a resource" in {
      val request =
        Get(s"$baseApiUrl/v2/resources/history/${URLEncoder
          .encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing-with-history", "UTF-8")}")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val expectedAnswerJSONLD =
    ...
    "return a graph of resources reachable via links from/to a given resource" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=both")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
    ...
    "return a graph of resources reachable via links from a given resource" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=outbound")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val parsedReceivedJsonLD = JsonLDUtil.parseJsonLD(responseAsString)
    ...
    "return a graph of resources reachable via links to a given resource" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=inbound")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val parsedReceivedJsonLD = JsonLDUtil.parseJsonLD(responseAsString)
    ...
    "return a graph of resources reachable via links to/from a given resource, excluding a specified property" in {
      val request = Get(
        s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=both&excludeProperty=${URLEncoder
    ...
    "return a graph of resources reachable via links from a given resource, specifying search depth" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=both&depth=2")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.OK, responseAsString)
      val parsedReceivedJsonLD = JsonLDUtil.parseJsonLD(responseAsString)
    ...
    "not accept a graph request with an invalid direction" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?direction=foo")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
    ...
    "not accept a graph request with an invalid depth (< 1)" in {
      val request = Get(
        s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?depth=0")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.BadRequest, responseAsString)
    }
    ...
    "not accept a graph request with an invalid depth (> max)" in {
      val request = Get(s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start",
    ...
    "not accept a graph request with an invalid property to exclude" in {
      val request =
        Get(
          s"$baseApiUrl/v2/graph/${URLEncoder.encode("http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/start", "UTF-8")}?excludeProperty=foo")
      val response: HttpResponse = singleAwaitingRequest(request)
      val responseAsString = responseToString(response)
      assert(response.status == StatusCodes.BadRequest, responseAsString)
    }
    ...
    "not create a resource with a custom IRI containing the wrong project code" in {
      val customIRI: IRI = "http://rdfh.ch/6NIOckxUlWnYChtXoH7A_g/a-thing-with-IRI"
      val jsonLDEntity = createResourceWithCustomIRI(customIRI)
      val request = Post(s"$baseApiUrl/v2/resources", HttpEntity(RdfMediaTypes.`application/ld+json`, jsonLDEntity)) ~> addCredentials(
        BasicHttpCredentials(anythingUserEmail, password))
      val response: HttpResponse = singleAwaitingRequest(request)
    ...
    "return a DuplicateValueException during resource creation when the supplied resource IRI is not unique" in {
      ...
      val invalidIri: Boolean =
        errorMessage.contains(s"IRI: 'http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-thing' already exists, try another one.")
    ...
    "update the metadata of a resource" in {
      val resourceIri = "http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-thing"
    ...
    "create a resource containing a text value with multiple standoff links" in {
      val jsonLDEntity =
        """{
          ...
          |  "anything:hasText": {
          |  "@type": "knora-api:TextValue",
          |  "knora-api:textValueAsXml": "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<text>\n   This text links to another <a class=\"salsah-link\" href=\"http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/another-thing\">thing</a> and a <a class=\"salsah-link\" href=\"http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/a-blue-thing\">blue thing</a>.\n</text>",
          |   "knora-api:textValueHasMapping": {
          |                  "@id": "http://rdfh.ch/standoff/mappings/StandardMapping"
          |            	}
          |            },
    ...
    "return a IIIF manifest for the pages of a book" in {
      val resourceIri = "http://rdfh.ch/U7HxeFSUEQCHJxSLahw3AA/thing-with-pages"
