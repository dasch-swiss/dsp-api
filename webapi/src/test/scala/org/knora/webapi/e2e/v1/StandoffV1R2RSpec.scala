...
class StandoffV1R2RSpec extends R2RSpec {
    ...
    "The Standoff Endpoint" should {
        ...
        "create a mapping resource for standoff conversion for HTML-Extended" in {

            val mappingFileToSend = new File(RequestParams.pathToHTMLExtMapping)

            val formDataMapping = Multipart.FormData(
                Multipart.FormData.BodyPart(
                    "json",
                    HttpEntity(ContentTypes.`application/json`, RequestParams.paramsCreateHTMLExtMappingFromXML)
                ),
                Multipart.FormData.BodyPart(
                    "xml",
                    HttpEntity.fromPath(ContentTypes.`text/xml(UTF-8)`, mappingFileToSend.toPath),
            ...
            Post("/v1/mapping", formDataMapping) ~> addCredentials(BasicHttpCredentials(anythingUserEmail, password)) ~> standoffPath ~> check {
                ...
                assert(mappingIri == anythingProjectIri + "/mappings/HTMLExtMapping", "Iri of the new mapping is not correct")
        ...
        "create a TextValue from XML representing HTML Extended (in strict XML notation)" in {

            val xmlFileToSend = new File(RequestParams.pathToHTMLExt)
            ...
            val newValueParams =
                s"""
                {
                  ...
                  "richtext_value": {
                        "xml": ${JsString(Source.fromFile(xmlFileToSend).mkString)},
                        "mapping_id": "${anythingProjectIri}/mappings/HTMLExtMapping"
                  }
                }
                """
