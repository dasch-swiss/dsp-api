...
class ResourcesV1R2RSpec extends R2RSpec {
    ...
    "The Resources Endpoint" should {
        ...
        "not create a resource in the shared ontologies project in a bulk import" in {
            val xmlImport =
                s"""<?xml version="1.0" encoding="UTF-8"?>
                   |<knoraXmlImport:resources xmlns="http://api.knora.org/ontology/shared/example-box/xml-import/v1#"
                   |    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                   |    xsi:schemaLocation="http://api.knora.org/ontology/shared/example-box/xml-import/v1# shared-example-box.xsd"
                   |    xmlns:shared-example-box="http://api.knora.org/ontology/shared/example-box/xml-import/v1#"
                   |    xmlns:knoraXmlImport="http://api.knora.org/ontology/knoraXmlImport/v1#">
                   |    <shared-example-box:Box id="test_box">
                   |        <knoraXmlImport:label>test box</knoraXmlImport:label>
                   |        <shared-example-box:hasName knoraType="richtext_value">This is a test.</shared-example-box:hasName>
                   |    </shared-example-box:Box>
                   |</knoraXmlImport:resources>""".stripMargin
            ...
            Post(s"/v1/resources/xmlimport/$projectIri", HttpEntity(ContentType(MediaTypes.`application/xml`, HttpCharsets.`UTF-8`), xmlImport)) ~> addCredentials(BasicHttpCredentials(incunabulaUserEmail, password)) ~> resourcesPath ~> check {
                assert(status == StatusCodes.BadRequest, response.toString)
                val responseStr = responseAs[String]
                responseStr should include("Invalid XML import namespace")
            }
        }
