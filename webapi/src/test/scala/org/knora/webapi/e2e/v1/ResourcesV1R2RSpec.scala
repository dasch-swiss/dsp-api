...
class ResourcesV1R2RSpec extends R2RSpec {
  ...
  "The Resources Endpoint" should {
    ...
    "get the regions of a page when doing a context query with resinfo set to true" in {
      ...
      Get(
        "/v1/resources/http%3A%2F%2Frdfh.ch%2F0803%2F9d626dc76c03?resinfo=true&reqtype=context"
      ) ~> resourcesPathV1 ~> check {
    ...
    "change the created text value above for the first thing resource so it has a standoff link to incunabulaBookBiechlin" in {
      ...
      ) ~> addCredentials(BasicHttpCredentials(anythingUserEmail, password)) ~> valuesPathV1 ~> check {
        ...
        firstElement.fields.get("ext_res_id") match {
          case Some(extResObj: JsObject) =>
            ...
            propIriJsString match {
              case JsString(pid) =>
                assert(
                  pid == OntologyConstants.KnoraBase.HasStandoffLinkTo,
    ...
    "mark a resource as deleted" in {
      ...
      Delete(
        "/v1/resources/http%3A%2F%2Frdfh.ch%2F0803%2F9d626dc76c03?deleteComment=deleted%20for%20testing"
      ) ~> addCredentials(BasicHttpCredentials(incunabulaUserEmail2, password)) ~> resourcesPathV1 ~> check {
        assert(status == StatusCodes.OK, response.toString)
      }
    }
    ...
    "get the fifth resource of type anything:Thing, containing various standoff markup" in {
      ...
      Get("/v1/resources/" + URLEncoder.encode(fifthThingIri.get, "UTF-8")) ~> addCredentials(
        ...
        assert(
          links(1).attributes.asAttrMap("class") == "salsah-link"
        ) // The link to a resource IRI has class="salsah-link"
    ...
    "change a resource's label" in {
      ...
      Put(
        "/v1/resources/label/" + URLEncoder.encode("http://rdfh.ch/0803/c5058f3a", "UTF-8"),
    ...
    "create resources from an XML import" in {
      ...
      val projectIri = URLEncoder.encode("http://rdfh.ch/projects/bL0y8GRuTUiFmvF1oXbeFQ", "UTF-8")

      Post(
        s"/v1/resources/xmlimport/$projectIri",
        HttpEntity(ContentType(MediaTypes.`application/xml`, HttpCharsets.`UTF-8`), xmlImport)
      ) ~> addCredentials(BasicHttpCredentials(beolUserEmail, password)) ~> resourcesPathV1 ~> check {
    ...
    "create an anything:Thing with all data types from an XML import" in {
      ...
      Post(
        s"/v1/resources/xmlimport/$projectIri",
        HttpEntity(ContentType(MediaTypes.`application/xml`, HttpCharsets.`UTF-8`), xmlImport)
      ) ~> addCredentials(BasicHttpCredentials(anythingAdminEmail, password)) ~> resourcesPathV1 ~> check {
        val responseStr = responseAs[String]
        assert(status == StatusCodes.OK, responseStr)
        responseStr should include("createdResources")
      }
    ...
    "not create a resource in a shared ontologies project in a bulk import" in {
      ...
      Post(
        s"/v1/resources/xmlimport/$projectIri",
        HttpEntity(ContentType(MediaTypes.`application/xml`, HttpCharsets.`UTF-8`), xmlImport)
      ) ~> addCredentials(BasicHttpCredentials(superUserEmail, password)) ~> resourcesPathV1 ~> check {
        assert(status == StatusCodes.BadRequest, response.toString)
        val responseStr = responseAs[String]
    ...
    "create 10,000 anything:Thing resources with random contents" in {
      ...
      Post(
        s"/v1/resources/xmlimport/$projectIri",
        HttpEntity(ContentType(MediaTypes.`application/xml`, HttpCharsets.`UTF-8`), xmlStringBuilder.toString)
      ) ~> addCredentials(BasicHttpCredentials(anythingAdminEmail, password)) ~> resourcesPathV1 ~> check {
        val responseStr = responseAs[String]
        assert(status == StatusCodes.OK, responseStr)
        responseStr should include("createdResources")
