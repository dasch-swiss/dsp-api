...
class SearchRouteV2R2RSpec extends R2RSpec {
    ...
    "The Search v2 Endpoint" should {
        ...
        "do a Gravsearch query in which 'rdf:type knora-api:Resource' is inferred from a more specific rdf:type" in {
            val gravsearchQuery =
                """
                  |PREFIX beol: <http://0.0.0.0:3333/ontology/0801/beol/simple/v2#>
                  |PREFIX knora-api: <http://api.knora.org/ontology/knora-api/simple/v2#>
        ...
        "do a Gravsearch query in which the object types of property IRIs are inferred" in {
            val gravsearchQuery =
                """
                  |PREFIX beol: <http://0.0.0.0:3333/ontology/0801/beol/simple/v2#>
                  |PREFIX knora-api: <http://api.knora.org/ontology/knora-api/simple/v2#>
        ...
        "do a Gravsearch query in which the types of property objects are inferred from the knora-api:objectType of each property" in {
            val gravsearchQuery =
                """
                  |PREFIX beol: <http://0.0.0.0:3333/ontology/0801/beol/simple/v2#>
                  |PREFIX knora-api: <http://api.knora.org/ontology/knora-api/simple/v2#>
        ...
        "do a Gravsearch query in which the knora-api:objectType of a property variable is inferred from a FILTER" in {
            val gravsearchQuery =
                """
                  |PREFIX beol: <http://0.0.0.0:3333/ontology/0801/beol/simple/v2#>
                  |PREFIX knora-api: <http://api.knora.org/ontology/knora-api/simple/v2#>
            ...
            Post("/v2/searchextended", HttpEntity(SparqlQueryConstants.`application/sparql-query`, gravsearchQuery)) ~> searchPath ~> check {
                ...
                val expectedAnswerJSONLD = FileUtil.readTextFile(new File("src/test/resources/test-data/searchR2RV2/letterWithPersonWithoutName.jsonld"))
