    ...
    "During Authentication" when {
        "called, the 'getUserADMByEmail' method " should {
            "succeed with the correct 'email' " in {
                val resF = Authenticator invokePrivate getUserByIdentifier(UserIdentifierADM(email = Some(AuthenticatorSpec.rootUserEmail)), system, responderManager, timeout, executionContext)
                resF map { res => assert(res == AuthenticatorSpec.rootUser)}
            }

            "fail with the wrong 'email' " in {
                    val resF = Authenticator invokePrivate getUserByIdentifier(UserIdentifierADM(email = Some("wronguser@example.com")), system, responderManager, timeout, executionContext)
        ...
        "called, the 'authenticateCredentialsV2' method" should {
            "succeed with correct email/password" in {
                val correctPasswordCreds = KnoraPasswordCredentialsV2(UserIdentifierADM(email = Some(AuthenticatorSpec.rootUserEmail)), AuthenticatorSpec.rootUserPassword)
            ...
            "fail with unknown email" in {
                    val wrongPasswordCreds = KnoraPasswordCredentialsV2(UserIdentifierADM(email = Some("wrongemail")), "wrongpassword")
            ...
            "fail with wrong password" in {
                    val wrongPasswordCreds = KnoraPasswordCredentialsV2(UserIdentifierADM(email = Some(AuthenticatorSpec.rootUserEmail)), "wrongpassword")
                    val resF = Authenticator invokePrivate authenticateCredentialsV2(Some(wrongPasswordCreds), system, responderManager, executionContext)
                    resF map {res => assertThrows(BadCredentialsException)}
            }
            "succeed with correct token" in {
                ...
                val resF = Authenticator invokePrivate authenticateCredentialsV2(Some(tokenCreds), system, responderManager, executionContext)
                resF map {res => assert(res)}
            }
