...
object CardinalityServiceLiveSpec extends ZIOSpecDefault {
    ...
    object Gens {
      ...
      val knoraOntologiesGen: Gen[Any, TestIris] = Gen.fromZIO {
        val values = Array(
          ...
          (
            OntologyIri.unsafeFrom(o.value.toSmartIri),
            ResourceClassIri.unsafeFrom(c.value.toSmartIri),
            ResourceClassIri.unsafeFrom(sc.value.toSmartIri),
            PropertyIri.unsafeFrom(p.value.toSmartIri),
          ),
        )
        Random
    ...
    suite("CardinalityServiceLive")(
      suite("canSetCardinality")(
          ...
          check(CanSetCardinalityTestData.Gens.knoraOntologiesGen) { iris =>
            check(cardinalitiesGen()) { cardinality =>
              val d = CanSetCardinalityTestData.createOntologyWithSuperClassCardinality(
                cardinality,
                iris.ontologyIri.toInternalIri,
                iris.classIri.toInternalIri,
                iris.subClassIri.toInternalIri,
                iris.propertyIri.toInternalIri,
              )
              for {
        ...
        test("Given no super-class or subclass exists then setting a cardinality is possible") {
          val classIri    = ResourceClassIri.unsafeFrom(CanSetCardinalityTestData.thingSmartIri)
          val propertyIri = PropertyIri.unsafeFrom(CanSetCardinalityTestData.hasValueSmartIri)
          ...
          check(cardinalitiesGen()) { cardinality =>
            for {
              ...
              actual <- ZIO.serviceWithZIO[CardinalityService](_.canSetCardinality(classIri, propertyIri, cardinality))
            ...
            test(
              ...
              check(cardinalitiesGen(AtLeastOne, Unbounded, ZeroOrOne)) { newCardinality =>
                ...
                for {
                  ...
                  actual <- ZIO.serviceWithZIO[CardinalityService](
                              _.canSetCardinality(
                                toResourceClassIri(d.subclassIri),
                                toPropertyIri(d.propertyIri),
                                newCardinality,
                              ),
                            )
              ...
              for {
                ...
                actual <-
                  ZIO.serviceWithZIO[CardinalityService](
                    _.canSetCardinality(toResourceClassIri(d.subclassIri), toPropertyIri(d.propertyIri), ExactlyOne),
                  )
              } yield assertTrue(actual.isRight)
            },
          ),
          ...
          test(
            ...
            check(cardinalitiesGen()) { newCardinality =>
              ...
              for {
                ...
                actual <-
                  ZIO.serviceWithZIO[CardinalityService](
                    _.canSetCardinality(toResourceClassIri(d.subclassIri), toPropertyIri(d.propertyIri), newCardinality),
                  )
              } yield assertTrue(actual.isRight)
            }
          },
        ...
        suite("Check subclass")(
          test(
            ...
            for {
              ...
              actual <-
                ZIO.serviceWithZIO[CardinalityService](
                  _.canSetCardinality(toResourceClassIri(d.subclassIri), toPropertyIri(d.propertyIri), Unbounded),
                )
            } yield assertTrue(actual.isRight)
          },
          test(
          ...
          check(cardinalitiesGen(ZeroOrOne, ExactlyOne)) { newCardinality =>
            for {
              ...
              actual <-
                ZIO.serviceWithZIO[CardinalityService](
                  _.canSetCardinality(
                    toResourceClassIri(Anything.Class.Thing),
                    toPropertyIri(Anything.Property.hasOtherThing),
                    newCardinality,
                  ),
                )
            } yield assertTrue(
      ...
      suite("CardinalityServiceLive persistence check")(test(s"""
        ...
        for {
          ...
          result <- ZIO.serviceWithZIO[CardinalityService](
                      _.canSetCardinality(
                        toResourceClassIri(Biblio.Class.Article),
  ...
  private def toResourceClassIri(iri: InternalIri): ResourceClassIri = ResourceClassIri.unsafeFrom(iri.value.toSmartIri)

  private def toPropertyIri(iri: InternalIri): PropertyIri = PropertyIri.unsafeFrom(iri.value.toSmartIri)
