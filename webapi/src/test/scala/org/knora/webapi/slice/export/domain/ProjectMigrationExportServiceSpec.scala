...
object ProjectMigrationExportServiceSpec extends ZIOSpecDefault {
  ...
  private def stubTriplestoreService: TriplestoreService = new TriplestoreService {
  ...
  private val makeTestEnv: ZIO[Any, Nothing, TestEnv] = for {
    ...
    triplestore          = stubTriplestoreService
  ...
  } yield TestEnv(service, storage, exportProjectCalled)
