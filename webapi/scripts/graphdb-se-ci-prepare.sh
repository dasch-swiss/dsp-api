...
# Create repository
curl  -s -S -X POST -H "Content-Type:text/turtle" --data-binary @./graphdb-se-knora-test-unit-repository-config.ttl http://localhost:8080/openrdf-sesame/repositories/SYSTEM/rdf-graphs/service?graph=http://www.knora.org/config-test-unit
...
curl -s -S -X POST -H "Content-Type:text/turtle" --data-binary @./graphdb-se-knora-test-unit-index-config.ttl http://localhost:8080/openrdf-sesame/repositories/knora-test-unit/statements
