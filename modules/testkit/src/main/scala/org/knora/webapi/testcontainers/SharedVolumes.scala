...
object SharedVolumes {
  ...
  object Images {
    ...
    /**
     * `/sipi/testfiles` is a classpath resource *directory*. In a non-Bazel run it resolves to a
     * real filesystem directory (loose class/resource output), so `Path(uri)` + `Files.walk` works
