...
object AddMappingQuery {
  ...
  private def buildUpdate(
    ...
    val pred        = predicate match
      case MappingPredicate.SubClassOf    => sparql"rdfs:subClassOf"
      case MappingPredicate.SubPropertyOf => sparql"rdfs:subPropertyOf"

    val mappingTriples =
      externalObjectIris.map(ext => sparql"$subject $pred ${Iri.unsafeFrom(ext.value)} .").joinLines
