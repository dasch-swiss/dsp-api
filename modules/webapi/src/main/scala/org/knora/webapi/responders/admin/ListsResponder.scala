      ...
      query = createNodeRequest match {
                ...
                case c: ListCreateChildNodeRequest =>
                  CreateListNodeQuery.createChildNode(
                    project = project,
                    node = newListNodeIri,
                    parentNode = c.parentNodeIri,
                    rootNode = rootNodeIri.get,
                    position = Position.unsafeFrom(newPosition.get),
                    name = c.name,
                    labels = c.labels,
                    comments = c.comments,
                  )
  ...
  def nodePositionChangeRequest(
    ...
    def updateParentAndPosition(
      ...
      for {
        ...
        _ <-
          shiftNodes(currentNodePosition + 1, currentSiblings.last.position, currentSiblings, project, ShiftLeft)
