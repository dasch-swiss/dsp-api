  ...
  def build(
    ...
    ZIO
      ...
      .as {
        ...
        val deleteLinkPatterns = indexed.map { case (lu, i) =>
          ...
          val linkValue         = linkValueOf(i)
          sparql"""|${sparql"$resource $linkProperty $linkTarget .".when(lu.deleteDirectLink)}
                   |$resource $linkValueProperty $linkValue .
                   |$linkValue knora-base:valueHasUUID ${linkUuidOf(i)} ."""
        }.joinLines

        val commentInsert =
          maybeDeleteComment.whenSome(c => sparql"$value knora-base:deleteComment ${Literal.string(c)} .")

        // Add a new LinkValue version for each resource reference in standoff markup.
        val insertLinkPatterns = indexed.map { case (lu, i) =>
          val linkProperty      = Iri.unsafeFrom(lu.linkPropertyIri.toInternalSchema.toIri)
          val linkValueProperty = Iri.unsafeFrom(lu.linkPropertyIri.toInternalSchema.toIri + "Value")
          val linkTarget        = Iri.unsafeFrom(lu.linkTargetIri)
        ...
        val whereLinkPatterns = indexed.map { case (lu, i) =>
          val linkProperty      = Iri.unsafeFrom(lu.linkPropertyIri.toInternalSchema.toIri)
          val linkValueProperty = Iri.unsafeFrom(lu.linkPropertyIri.toInternalSchema.toIri + "Value")
          val linkTarget        = Iri.unsafeFrom(lu.linkTargetIri)
