...
private[service] object ValueQueries {
  ...
  def reorderValues(
    ...
    // rows are assembled in the hole below. An empty list would render an empty — but still legal —
    // VALUES block; callers always pass at least one value.
    Update(
      sparql"""|PREFIX knora-base: <http://www.knora.org/ontology/knora-base#>
               |
               |WITH $graph
               ...
               |    ${orderedValueIris.zipWithIndex.map { case (valueIri, index) =>
