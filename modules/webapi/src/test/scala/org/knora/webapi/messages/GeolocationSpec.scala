 ...
 * Test names carry their REQ ids, so `grep -rn "REQ-"` maps coverage back to the PRD.
 */
...
class GeolocationSpec extends ZIOSpecDefault {
  ...
  val spec: Spec[Any, Nothing] = suite("Geolocation")(
    suite("accepts")(
      ...
      test("REQ-5.8: an untagged literal, storing it with an explicit CRS84 prefix") {
        val actual = accepted("POINT(8.55 47.37)")
        assertTrue(actual.crs == Crs.Crs84, actual.toStoredLiteral == s"<$crs84> POINT(8.55 47.37)")
      },
      test("REQ-5.10: coordinates with their submitted decimal precision, trailing zeros included") {
      ...
      test("REQ-5.11: the geometry keyword case-insensitively, and leading and trailing whitespace") {
        assertTrue(
      ...
      test("REQ-5.9: the WGS84 bounds inclusively") {
        assertTrue(
      ...
      test("REQ-5.12: -0.0 as equal to 0.0") {
    ...
    suite("rejects")(
      test("REQ-5.3: EPSG:4326, naming CRS84 as the replacement") {
        val message = rejection(s"<${Crs.RejectedEpsg4326}> POINT(8.55 47.37)")
        assertTrue(message.contains(crs84), message.contains("latitude"))
      },
      test("REQ-5.2: a CRS outside the allowlist, listing what is supported") {
      ...
      test("REQ-5.1: a literal that is not a CRS IRI in angle brackets followed by a WKT geometry") {
        assertTrue(
      ...
      test("REQ-5.6: a third ordinate, whether tagged POINT Z or merely present") {
        assertTrue(
          rejection(s"<$crs84> POINT Z (8.55 47.37 400)").contains("POINT Z"),
          rejection(s"<$crs84> POINT(8.55 47.37 400)").contains("ordinates"),
        )
      },
      test("REQ-5.7: a POINT carrying no coordinate pair") {
        assertTrue(
    ...
    suite("the accepted gap (PRD §3.2)")(
    ...
    suite("valueHasString")(
      test("REQ-4.6: is the bare space-separated coordinates, without CRS prefix or geometry wrapper") {
        assertTrue(accepted(s"<$crs84> POINT(8.550 47.37)").coordinates == "8.550 47.37")
      },
    ),
    suite("decompose (the lenient read path)")(
      test("REQ-4.1, REQ-4.2: splits a stored point into CRS, shape and coordinates") {
      ...
      test("REQ-7.1: never fails on a line or an area, which the write path rejects") {
      ...
      test("REQ-7.2: never fails on an elevation") {
