...
class DeleteResourceQuerySpec extends ZIOSpecDefault {
  ...
  override def spec: Spec[TestEnvironment, Any] = suite("DeleteResourceQuery")(
    test("build should produce the expected SPARQL query without delete comment") {
      val actual = DeleteResourceQuery.build(
        project = testProject,
      ...
      val expected =
        s"""|PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
            ...
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:lastModificationDate ?resourceLastModificationDate .
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:isDeleted false .
            |  }
            |}
            |INSERT {
            |  GRAPH <$dataNamedGraph> {
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:isDeleted true .
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:deletedBy <http://rdfh.ch/users/root> .
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:deleteDate "$currentTime"^^<http://www.w3.org/2001/XMLSchema#dateTime> .
            |    <http://rdfh.ch/0001/thing-with-history> knora-base:lastModificationDate "$currentTime"^^<http://www.w3.org/2001/XMLSchema#dateTime> .
            |  }
            |}
            |WHERE {
            |  <http://rdfh.ch/0001/thing-with-history> rdf:type ?resourceClass .
            |  <http://rdfh.ch/0001/thing-with-history> knora-base:isDeleted false .
            ...
            |  }
            |}""".stripMargin
      val parsed = UpdateFactory.create(actual)

      assertTrue(actual == expected, !parsed.getOperations.isEmpty)
    },
