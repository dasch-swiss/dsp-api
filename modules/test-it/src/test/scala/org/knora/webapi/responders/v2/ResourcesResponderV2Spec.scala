...
object ResourcesResponderV2Spec extends E2EZSpec { self =>
  ...
  override val e2eSpec = suite("ResourceResponserV2")(
    ...
    suite("The resources responder v2")(
      ...
      //   } yield assertCompletes
      // },
      test("create a resource with a region preview value") {
        val regionIri     = ResourceIri.unsafeFrom("http://rdfh.ch/0001/A5NfXW4QRxOnBPULCTvH5w")
        val resourceIri   = ResourceIri.makeNew(anythingProject.shortcode)
        val propertyIri   = "http://0.0.0.0:3333/ontology/0001/anything/v2#hasRegionPreview".toSmartIri
        val inputResource = CreateResourceV2(
          resourceIri = Some(resourceIri),
          resourceClassIri = "http://0.0.0.0:3333/ontology/0001/anything/v2#Thing".toSmartIri,
          label = "test thing with region preview",
          values = Map(
            propertyIri -> Seq(
              CreateValueInNewResourceV2(
        ...
        for {
          ...
          _                    <- ZIO.consoleWith(_.printLine(s"ResourcesResponderV2Spec:1197: $outputResource"))
          regionPreviewContent <-
            ZIO
              .fromOption(outputResource.values.get(propertyIri).flatMap(_.headOption).map(_.valueContent))
              .orElseFail(AssertionException("No region preview value found"))
              .flatMap {
                case v: RegionPreviewValueContentV2 => ZIO.succeed(v)
                case other                          =>
                  ZIO.fail(AssertionException(s"Expected RegionPreviewValueContentV2, got $other"))
              }
        } yield assertTrue(regionPreviewContent.regionIri == regionIri)
      },
