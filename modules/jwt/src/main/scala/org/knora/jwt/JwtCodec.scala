...
object JwtCodec {
  ...
  private def hmacSha256(data: String, secret: String): Array[Byte] = {
   ...
   * @param secret    the HMAC-SHA256 secret
   * @return the encoded JWT string
   */
  def encode(header: String, claimJson: String, secret: String): String = {
   ...
   * @return the encoded JWT string
   */
  def encode(claim: JwtClaim, secret: String): String =
   ...
   */
  def decode(token: String, secret: String): Try[JwtClaim] =
   ...
   */
  def decodeAll(token: String, secret: String): Try[(JwtHeader, JwtClaim, String)] = {
