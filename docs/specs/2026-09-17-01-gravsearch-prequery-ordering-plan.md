...
- spike write-up = `<spec-dir>/<stem>-design.md` (discovery, per-case results, tier table); raw spike files
  (runner script, layout `.rq` files, `results.csv`) = `<spec-dir>/<stem>-assets/` (committed; *revised at ship
  time*: Balduin decided not to track the raw files, they live under `~/Desktop/gravsearch-ordering-measurements/`)
...
| D12 | Plan location | `docs/specs/` in the repo under the SIPI spec convention (Balduin, 2026-09-17: "move the plan to the repo", "how does sipi handle this?"); journal (`-journal.md`), spike write-up (`-design.md`) and raw spike files (`-assets/`) share the plan's stem and are committed at ship time (*revised 2026-09-18: the raw files are kept outside the repository, the design doc's tables are the record*); review diffs stay under `.claude/tmp` | keeps the measured basis and the execution journal with the code; `docs/specs/` is excluded from the mkdocs site, passes `just check` (markdownlint), and is a reference sink (no inbound links). |
