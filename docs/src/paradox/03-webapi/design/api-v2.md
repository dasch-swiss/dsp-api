...
## General Principles
  ...
  - Knora API v2 requests and responses are RDF documents. Currently
    only [JSON-LD](http://json-ld.org/) documents are supported. Support
    for XML is planned.
  - Each class or property used in a request or response has a
    definition in an ontology, which the Knora API server can serve.
...
@ref:[Knora IRIs](../api-v2/knora-iris.md).
...
A route calls `RouteUtilV2.runJsonRoute`, passing a request message and
...
`StringFormatter` class, because it uses the Knora API server's
