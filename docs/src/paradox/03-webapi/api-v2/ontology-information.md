...
The project IRI must be URL-encoded. Example response for the `anything`
test project (project IRI `http://rdfh.ch/projects/0001`):
...
The ontology IRI must be URL-encoded, and may be in either the default
...
The default schema is suitable for client applications that need to
update data in Knora. For example, here is the response for the `images`
sample ontology in the default schema,
`http://0.0.0.0:3333/ontology/00FF/images/v2` (simplified for clarity):
...
In the default schema, all Knora value properties are object properties,
...
by requesting the `knora-api` ontology in the default schema,
`http://api.knora.org/ontology/knora-api/v2`.

Moreover, additional information is provided in the default schema, to
...
default schema. If the new property is a subproperty of
