...
The \texttt{StandoffTag} class is not used directly in RDF data; instead, its subclasses are used. A few subclasses are currently provided, and more will be added to support TEI semantics. A project can define its own custom standoff tag classes.
...
Associates a complex value, such as a date, with a substring in a text. There is currently one subclass:

\begin{itemize}
	\item \texttt{StandoffDateValueTag} Represents a date, in the same form that is used for \texttt{kb:DateValue}.
\end{itemize}

\subsubsection{StandoffReferenceTag}

Represents a reference to data outside the text. Subclasses:

\begin{itemize}
	\item \texttt{StandoffHrefTag} Indicates that a substring is associated with a resource on the Internet, i.e.\ a URL.
	\item \texttt{StandoffLinkTag} Indicates that a substring is associated with a Knora resource. For example, if a repository contains resources representing persons, a text could be marked up so that each time a person's name is mentioned, a \texttt{StandoffLinkTag} connects the name to the Knora resource describing that person. See the next section for details.
\end{itemize}

\subsubsection{Standoff Links}
...
\emph{The features in this section are under development.}
