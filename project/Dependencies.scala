...
object Dependencies {
  ...
  val fusekiImage =
    "daschswiss/apache-jena-fuseki:2.1.3" // should be the same version as in docker-compose.yml, also make sure to use the same version when deploying it (i.e. version in ops-deploy)!
