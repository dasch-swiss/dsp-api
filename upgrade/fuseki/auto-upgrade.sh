...
curl -X GET -H "Accept: application/trig" -u "${USERNAME}:${PASSWORD}" "http://${HOST}/fuseki/${REPOSITORY}" > "${INPUT_FILE}"
...
if [[ -f "${OUTPUT_FILE}" ]]; then
    ...
    curl -X POST -H "Content-Type: application/sparql-update" -d "DROP ALL" -u "${USERNAME}:${PASSWORD}" "http://${HOST}/fuseki/${REPOSITORY}"

    echo "Uploading transformed data to repository..."
    curl -X POST -H "Content-Type: application/trig" --data-binary "@${OUTPUT_FILE}" -u "${USERNAME}:${PASSWORD}" "http://${HOST}/fuseki/${REPOSITORY}" | tee /dev/null
fi
