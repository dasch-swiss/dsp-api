...
from plugins.pr1372.update import GraphTransformer
...
def test_update():
    ...
    input_graph.parse("plugins/pr1372/test_data.ttl", format="turtle")
    ...
    values_with_permissions = set([row["value"] for row in query_result])

    assert values_with_permissions == {
        URIRef('http://rdfh.ch/0001/thing-with-history/values/1c'),
        URIRef('http://rdfh.ch/0001/thing-with-history/values/2c'),
        URIRef('http://rdfh.ch/0001/thing-with-history/values/3b')
    }
